...
#ifdef MINIUPNPC_SET_SOCKET_TIMEOUT
static void set_socket_timeouts(SOCKET s)
{
#ifdef _WIN32
	// Winsock uses a DWORD containing milliseconds, unlike POSIX which uses
	// struct timeval. Passing a timeval here reduced the intended 3 seconds to
	// 3 milliseconds on Windows.
	const DWORD timeout = 3000;
	if(setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (const char *)&timeout, (int)sizeof(timeout)) < 0)
	{
		PRINT_SOCKET_ERROR("setsockopt SO_RCVTIMEO");
	}
	if(setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (const char *)&timeout, (int)sizeof(timeout)) < 0)
	{
...
#if defined(MINIUPNPC_SET_SOCKET_TIMEOUT) && !defined(_WIN32)
	struct timeval timeout;
#endif /* MINIUPNPC_SET_SOCKET_TIMEOUT && !_WIN32 */
