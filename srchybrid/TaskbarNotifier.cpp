...
bool CTaskbarNotifier::LoadConfiguration(LPCTSTR pszFilePath)
{
	...
	if (strBmpFilePath.IsEmpty() || !SetBitmap(strBmpFilePath, iBmpTransparentRed, iBmpTransparentGreen, iBmpTransparentBlue)) {
		if (!CreateDefaultBitmap())
			return false;
		strFontType = _T("Segoe UI");
		iFontSize = 90;
		m_crNormalTextColor = CNextTheme::GetTheme() == entModernAurora ? RGB(31, 58, 67) : RGB(35, 49, 64);
		m_crSelectedTextColor = CNextTheme::GetTheme() == entModernAurora ? RGB(31, 132, 140) : RGB(37, 119, 181);
		SetTextRect(CRect(52, 38, 292, 88));
		SetCloseBtnRect(CRect(294, 9, 316, 31));
		SetHistoryBtnRect(CRect(264, 9, 289, 31));
	}
...
bool CTaskbarNotifier::CreateDefaultBitmap()
{
	...
	CBitmap bitmap;
	if (!bitmap.CreateCompatibleBitmap(&screenDC, 324, 104))
		return false;
	CBitmap *pOldBitmap = dc.SelectObject(&bitmap);

	const EEmuleNextTheme eTheme = CNextTheme::GetTheme();
	const COLORREF crMask = RGB(255, 0, 255);
	const COLORREF crSurface = eTheme == entModernAurora ? RGB(244, 249, 250) : RGB(248, 250, 252);
	const COLORREF crHeader = eTheme == entModernAurora ? RGB(48, 167, 173) : RGB(51, 145, 210);
	const COLORREF crBorder = eTheme == entModernAurora ? RGB(129, 194, 198) : RGB(145, 188, 218);
	dc.FillSolidRect(0, 0, 324, 104, crMask);

	CPen borderPen(PS_SOLID, 1, crBorder);
	CBrush surfaceBrush(crSurface);
	CPen *pOldPen = dc.SelectObject(&borderPen);
	CBrush *pOldBrush = dc.SelectObject(&surfaceBrush);
	dc.RoundRect(CRect(1, 1, 323, 103), CPoint(15, 15));
	dc.SelectObject(pOldBrush);
	dc.SelectObject(pOldPen);

	CBrush headerBrush(crHeader);
	pOldBrush = dc.SelectObject(&headerBrush);
	pOldPen = static_cast<CPen*>(dc.SelectStockObject(NULL_PEN));
	dc.RoundRect(CRect(2, 2, 322, 42), CPoint(14, 14));
	dc.FillSolidRect(2, 25, 320, 17, crHeader);
	dc.SelectObject(pOldPen);
	dc.SelectObject(pOldBrush);

	CFont titleFont;
	titleFont.CreatePointFont(90, _T("Segoe UI Semibold"), &dc);
	CFont *pOldFont = dc.SelectObject(&titleFont);
	dc.SetBkMode(TRANSPARENT);
	dc.SetTextColor(RGB(255, 255, 255));
	dc.DrawText(_T("eMule Next"), CRect(14, 7, 250, 31), DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

	CPen glyphPen(PS_SOLID, 2, RGB(255, 255, 255));
	pOldPen = dc.SelectObject(&glyphPen);
	dc.MoveTo(301, 15); dc.LineTo(309, 23);
	dc.MoveTo(309, 15); dc.LineTo(301, 23);
	dc.MoveTo(271, 16); dc.LineTo(282, 16);
	dc.MoveTo(271, 20); dc.LineTo(282, 20);
	dc.MoveTo(271, 24); dc.LineTo(279, 24);
	dc.SelectObject(pOldPen);
	dc.SelectObject(pOldFont);
	dc.SelectObject(pOldBitmap);

	return SetBitmap(&bitmap, 255, 0, 255);
}
