...
private:
	static CString	_GetHeader(const ThreadData &Data, const CString &sSession);
	...
	static bool		_IsLoggedIn(const ThreadData &Data, const CString &sSession);
	static void		_RemoveTimeOuts(const ThreadData &Data);
	static bool		_RemoveSession(const ThreadData &Data, const CString &sSession);
	...
	static Session	_GetSessionByID(const ThreadData &Data, const CString &sSession);
	static bool		_IsSessionAdmin(const ThreadData &Data, const CString &sSession);
	...
	static void		_SetLastUserCat(const ThreadData &Data, const CString &sSession, int cat);
	static int		_GetLastUserCat(const ThreadData &Data, const CString &sSession);
	static void		_MakeTransferList(CString &Out, CWebServer *pThis, const ThreadData &Data, CArray<DownloadFiles> &filesArray, CArray<UploadUsers> &uploadArray, bool bAdmin);
	...
	CCriticalSection m_csSessionData;
	int				m_iSearchSortby;
