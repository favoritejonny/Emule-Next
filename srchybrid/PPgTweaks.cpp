...
void CPPgTweaks::DoDataExchange(CDataExchange *pDX)
{
	...
	bool bSparseFileSupportAvailable = false;
	for (INT_PTR i = 0; i < thePrefs.GetTempDirCount(); ++i) {
		if (VolumeSupportsSparseFiles(thePrefs.GetTempDir(i))) {
			bSparseFileSupportAvailable = true;
			break;
		}
	}
	m_ctrlTreeOptions.SetCheckBoxEnable(m_htiSparsePartFiles, wv != _WINVER_VISTA_ && bSparseFileSupportAvailable);
	...
	const bool bPortableMode = thePrefs.IsPortableMode();
	m_ctrlTreeOptions.SetRadioButtonEnable(m_htiShareeMuleMultiUser, !bPortableMode);
	m_ctrlTreeOptions.SetRadioButtonEnable(m_htiShareeMuleOldStyle, !bPortableMode);
	m_ctrlTreeOptions.SetRadioButtonEnable(m_htiShareeMulePublicUser, !bPortableMode && wv >= _WINVER_VISTA_);
	m_ctrlTreeOptions.SetCheckBoxEnable(m_htiAutoTakeEd2kLinks, !bPortableMode);
}
...
BOOL CPPgTweaks::OnApply()
{
	...
	if (!thePrefs.IsPortableMode() && thePrefs.AutoTakeED2KLinks() != m_bAutoTakeEd2kLinks) {
