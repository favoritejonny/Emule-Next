...
void CTreePropSheet::UpdateCaption()
{
	...
	HICON hIcon = NULL;
	if (m_bTreeImages) {
		// Use the image list currently assigned to the tree. A consumer may
		// replace it after creation (as eMule Next does for modern icons), so
		// m_Images is not necessarily the list the user is seeing.
		int	nImage;
		m_pwndPageTree->GetItemImage(hItem, nImage, nImage);
		CImageList* const pTreeImages = m_pwndPageTree->GetImageList(TVSIL_NORMAL);
		if (pTreeImages != NULL && nImage >= 0 && nImage < pTreeImages->GetImageCount())
			hIcon = pTreeImages->ExtractIcon(nImage);
	} else if (bRealPage) {
