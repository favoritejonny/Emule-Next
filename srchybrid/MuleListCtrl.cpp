...
void CMuleListCtrl::SetSortArrow(int iColumn, ArrowType atType)
{
	...
	if (iColumn >= 0 && pHeaderCtrl->GetItem(iColumn, &headerItem)) {
		...
		LPCTSTR iconName = _T("NEXT_SORT_DOWN");
		switch (atType) {
		case arrowUp: iconName = _T("NEXT_SORT_UP"); break;
		case arrowDoubleDown: iconName = _T("NEXT_SORT_DOUBLE_DOWN"); break;
		case arrowDoubleUp: iconName = _T("NEXT_SORT_DOUBLE_UP"); break;
		default: break;
		}

		CRect headerRect;
		pHeaderCtrl->GetClientRect(&headerRect);
		HDC screenDC = ::GetDC(pHeaderCtrl->m_hWnd);
		const int dpi = screenDC != NULL ? ::GetDeviceCaps(screenDC, LOGPIXELSX) : 96;
		if (screenDC != NULL)
			::ReleaseDC(pHeaderCtrl->m_hWnd, screenDC);
		const int iconSize = min(max(14, MulDiv(14, dpi, 96)), max(14, headerRect.Height() - 4));

		HICON sortIcon = NULL;
		CImageList sortImages;
		const bool iconReady = CreateNextModernIcon(iconName, iconSize, iconSize, &sortIcon)
			&& sortImages.Create(iconSize, iconSize, ILC_COLOR32, 1, 0)
			&& sortImages.Add(sortIcon) >= 0;
		if (sortIcon != NULL)
			VERIFY(::DestroyIcon(sortIcon));

		headerItem.fmt &= ~(HDF_IMAGE | HDF_BITMAP_ON_RIGHT | HDF_SORTUP | HDF_SORTDOWN);
		if (iconReady) {
			(void)pHeaderCtrl->SetImageList(&sortImages);
			m_imlHeaderCtrl.DeleteImageList();
			m_imlHeaderCtrl.Attach(sortImages.Detach());
			headerItem.mask |= HDI_IMAGE;
			headerItem.fmt |= HDF_IMAGE | HDF_BITMAP_ON_RIGHT;
			headerItem.iImage = 0;
		} else {
			headerItem.fmt |= GetSortAscending() ? HDF_SORTUP : HDF_SORTDOWN;
		}
...
int CMuleListCtrl::MoveItem(int iOldIndex, int iNewIndex)
{
	...
	// Copy strings of sub items. Keep the special callback value separate from
	// the allocated text so no pointer needs to serve two unrelated purposes.
	struct ListSubItem
	{
		CString *pText;
		bool bTextCallback;
	};
	CSimpleArray<ListSubItem> aSubItems;
		...
		for (int i = 1; i < m_iColumnsTracked; ++i) {
			...
			if (GetItem(&lvi1)) {
				if (lvi1.pszText == LPSTR_TEXTCALLBACK)
					subItem.bTextCallback = true;
				else
					subItem.pText = new CString(szText1);
			}
			aSubItems.Add(subItem);
		}
	}
	...
	if ((Style & LVS_OWNERDATA) == 0) {
		for (int i = 1; i < m_iColumnsTracked; ++i) {
			...
			lvi1.mask = LVIF_TEXT;
			lvi1.iSubItem = i;
			const ListSubItem &subItem = aSubItems[i - 1];
			if (iNewIndex != -1 && (subItem.pText != NULL || subItem.bTextCallback)) {
				if (subItem.bTextCallback)
					lvi1.pszText = LPSTR_TEXTCALLBACK;
				else
					lvi1.pszText = const_cast<LPTSTR>((LPCTSTR)*subItem.pText);
				DefWindowProc(LVM_SETITEMTEXT, iNewIndex, (LPARAM)&lvi1);
			}
			delete subItem.pText;
		}
	}
