	...
	if (!m_Files_map.IsEmpty() && theApp.downloadqueue) {
		...
		for (POSITION pos = m_Files_map.GetStartPosition(); pos != NULL;) {
			...
			if (!cur_file->IsKindOf(RUNTIME_CLASS(CPartFile))
				...
				|| ::GetFileAttributes(MakeLongPathCompatible(cur_file->GetFilePath())) == INVALID_FILE_ATTRIBUTES)
			{
...
{
	Publish();
	if (m_lastPublishED2KFlag && HasTickCountElapsed(::GetTickCount(), m_lastPublishED2K, ED2KREPUBLISHTIME)) {
		SendListToServer();
