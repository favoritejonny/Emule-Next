...
uint32 CUpDownClient::GetScore(bool sysvalue, bool isdownloading, bool onlybasevalue) const
{
	...
	if (onlybasevalue)
		...
		dwBaseValue += MIN2MS(HasTickCountElapsed(::GetTickCount(), m_dwUploadTime, MIN2MS(15)) ? 15 : 30);
...
void CUpDownClient::UpdateUploadingStatisticsData()
{
	...
	if (sentBytesFile > 0 || m_AverageUDR_hist.IsEmpty() || HasTickCountElapsed(curTick, m_AverageUDR_hist.Tail().timestamp, SEC2MS(1))) {
	...
	while (!m_AverageUDR_hist.IsEmpty() && HasTickCountElapsed(curTick, m_AverageUDR_hist.Head().timestamp, SEC2MS(10))) {
		m_nSumForAvgUpDataRate -= m_AverageUDR_hist.Head().datalen;
		m_AverageUDR_hist.RemoveHead();
	}

	// Calculate average speed for this slot
	const DWORD elapsed = m_AverageUDR_hist.IsEmpty() ? 0 : curTick - m_AverageUDR_hist.Head().timestamp;
	if (elapsed != 0 && GetUpStartTimeDelay() > SEC2MS(2))
		m_nUpDatarate = (UINT)(SEC2MS(m_nSumForAvgUpDataRate) / elapsed);
	else
		m_nUpDatarate = 0; // not enough data to calculate trustworthy speed

	// Check if it's time to update the display.
	if (HasTickCountElapsed(curTick, m_lastRefreshedULDisplay, MINWAIT_BEFORE_ULDISPLAY_WINDOWUPDATE + rand() % 800)) {
		// Update display
...
void CUpDownClient::AddRequestCount(const uchar *fileid)
{
	...
	for (POSITION pos = m_RequestedFiles_list.GetHeadPosition(); pos != NULL;) {
		...
		if (md4equ(cur_struct->fileid, fileid)) {
			if (!HasTickCountElapsed(curTick, cur_struct->lastasked, MIN_REQUESTTIME) && !GetFriendSlot()) {
