...
BOOL CSplashScreen::OnInitDialog()
{
	...
	VERIFY(m_imgSplash.Attach(theApp.LoadImage(_T("COMMUNITY_SPLASH"), _T("JPG"))));
...
void CSplashScreen::OnPaint()
{
	...
	if (m_imgSplash.GetSafeHandle()) {
		...
		if (dcMem.CreateCompatibleDC(&dc)) {
			...
			dc.FillSolidRect(rc, RGB(7, 22, 40));
			dc.FillSolidRect(0, rc.top, BM.bmWidth, 2, RGB(47, 207, 232));

			const int nOldBkMode = dc.SetBkMode(TRANSPARENT);
			const COLORREF crOldTextColor = dc.SetTextColor(RGB(244, 251, 255));
			...
			const CString strAppVersion(theApp.GetProductDisplayName());
			...
			dc.SetTextColor(RGB(142, 221, 235));
			const CString strProjectCredits(_T("A project by Jonny Favorite\r\nCopyright (C) 2002-2026 Merkur"));
			dc.DrawText(strProjectCredits, -1, &rc, DT_CENTER | DT_NOPREFIX | DT_WORDBREAK);
			if (pOldFont)
				dc.SelectObject(pOldFont);
			font.DeleteObject();
