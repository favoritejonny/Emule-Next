...
static void ClearZipCentralDirectoryEntries(CTypedPtrList<CPtrList, ZIP_CentralDirectory*> *entries)
{
	while (entries != NULL && !entries->IsEmpty()) {
		ZIP_CentralDirectory *entry = entries->RemoveHead();
	...
	, bool preview, bool bCreatePartFileCopy)
{
	...
	try {
		...
		if (success) {
			...
			theApp.QueueDebugLogLine(false, _T("Archive recovery: Part file size: %s, temp. archive file size: %s (%.1f%%)"), (LPCTSTR)CastItoXBytes(partFile->GetFileSize()), (LPCTSTR)CastItoXBytes(ulTempFileSize), partFile->GetFileSize() > 0ull ? (static_cast<double>(ulTempFileSize) * 100.0 / static_cast<double>(partFile->GetFileSize())) : 0.0);
	...
	, CArray<Gap_Struct> *paFilled, bool fullSize)
{
	...
	ClearZipCentralDirectoryEntries(pDir);
	delete pDir;
...
bool CArchiveRecovery::readZipCentralDirectory(CFile *zipInput, CTypedPtrList<CPtrList, ZIP_CentralDirectory*> *centralDirectoryEntries, CArray<Gap_Struct> *filled)
{
	try {
		...
		if (readUInt32(zipInput) != ZIP_END_CD_MAGIC) {
			...
			const ULONGLONG markerSearchLength = min(zipInput->GetLength(), 1046ull);
			zipInput->Seek(-static_cast<LONGLONG>(markerSearchLength), CFile::end);
			if (!scanForZipMarker(zipInput, NULL, ZIP_END_CD_MAGIC, markerSearchLength))
				return false;
			// Skip it again
			readUInt32(zipInput);
		}
		...
		while (readUInt32(zipInput) == ZIP_CD_MAGIC) {
			const uint32 centralDirectoryFixedSize = 42;
			if (zipInput->GetLength() - zipInput->GetPosition() < centralDirectoryFixedSize) {
				ClearZipCentralDirectoryEntries(centralDirectoryEntries);
				return false;
			}
			...
			const uint32 variableFieldSize = static_cast<uint32>(cdEntry->lenFilename) + cdEntry->lenExtraField + cdEntry->lenComment;
			if (variableFieldSize > zipInput->GetLength() - zipInput->GetPosition()) {
				delete cdEntry;
				ClearZipCentralDirectoryEntries(centralDirectoryEntries);
				return false;
			}
	...
	try {
		...
		while (available > 0) {
			...
			for (BYTE *foundPos = chunk; foundPos != NULL; ++foundPos) {
				...
				const RARFILEHDR *hdr = reinterpret_cast<const RARFILEHDR*>(checkCRC);
				if (input->Read(checkCRC, sizeof *hdr) != sizeof *hdr)
					continue;
				...
				const uint16 maxFileNameLength = (hdr->flags & 0x0200) ? static_cast<uint16>(MAX_PATH * 3) : MAX_PATH;
				if (lenFileName > maxFileNameLength)
					continue;
				fileName = new BYTE[lenFileName];
				if (input->Read(fileName, lenFileName) != lenFileName) {
					delete[] fileName;
					fileName = NULL;
					continue;
				}
...
{
	char blockmem[MAXACEHEADERSIZE];
	const size_t fixedHeaderSize = offsetof(ACE_BlockFile, COMM_SIZE) - offsetof(ACE_BlockFile, HEAD_TYPE);
	...
	try {
		...
		while (available > 0) {
			...
			for (BYTE *foundPos = chunk; foundPos != NULL; ++foundPos) {
				...
				if (input->Read(&headCRC, sizeof headCRC) != sizeof headCRC
					|| input->Read(&headSize, sizeof headSize) != sizeof headSize)
				{
					continue;
				}
				if (headSize < fixedHeaderSize
					|| headSize > MAXACEHEADERSIZE // header limit
					|| (lenChunk - chunkOffset) + available < headSize)	// header too big for my filled part
				{
					continue;
				}
				if (input->Read(blockmem, headSize) != headSize)
					continue;
				...
				// basic checks
				if (newblock->FNAME_SIZE > static_cast<size_t>(blockmem + newblock->HEAD_SIZE - mempos)) {
					delete newblock;
					continue;
				}
				...
				if (newblock->FNAME_SIZE > 0) {
					...
					newblock->FNAME[newblock->FNAME_SIZE] = '\0';
				...
				if (static_cast<size_t>(blockmem + newblock->HEAD_SIZE - mempos) >= sizeof newblock->COMM_SIZE) {
					memcpy(&newblock->COMM_SIZE, mempos, sizeof newblock->COMM_SIZE);
					mempos += sizeof(newblock->COMM_SIZE);

					if (newblock->COMM_SIZE > static_cast<size_t>(blockmem + newblock->HEAD_SIZE - mempos)) {
						delete newblock;
						continue;
					}
					if (newblock->COMM_SIZE > 0) {
						newblock->COMMENT = (char*)malloc(newblock->COMM_SIZE + static_cast<size_t>(1));
						if (newblock->COMMENT == NULL) {
							delete newblock;
							continue;
						}
						memcpy(newblock->COMMENT, mempos, newblock->COMM_SIZE);
						newblock->COMMENT[newblock->COMM_SIZE] = '\0';
...
void CArchiveRecovery::ISOReadDirectory(archiveScannerThreadParams_s *aitp, UINT32 startSec, CFile *isoInput, const CString &currentDirName)
{
	...
	const UINT32 fixedEntrySize = sizeof(ISO_FileFolderEntry) - sizeof(TCHAR*);
	isoInput->Seek(startSec * secSize, FILE_BEGIN);
	while (aitp->m_bIsValid) {
		ISO_FileFolderEntry *file = new ISO_FileFolderEntry;

		UINT32 blocksize = isoInput->Read(file, fixedEntrySize);
		if (blocksize != fixedEntrySize) {
			delete file;
			break;
		}
		...
		if (file->lenRecord < fixedEntrySize + file->nameLen + ((file->nameLen & 1) ? 0 : 1)
			|| (aitp->ai->isoInfos.iJolietUnicode && (file->nameLen & 1)))
		{
			delete file;
			break;
		}
		...
		const UINT bytesRead = isoInput->Read(file->name, file->nameLen);
		if (bytesRead != file->nameLen) {
			delete file;
			break;
		}
		blocksize += bytesRead;
		...
		const UINT32 skip = file->lenRecord - blocksize;
