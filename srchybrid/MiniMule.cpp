...
void CMiniMule::UpdateContent(UINT uUpDatarate, UINT uDownDatarate)
{
	...
	if (m_bResolveImages) {
		...
		ASSERT(uIconIdx < 9);
		const UINT ed2kState = min(uIconIdx / 3, 2u);
		const UINT kadState = min(uIconIdx % 3, 2u);
		CString statusHtml;
		statusHtml.Format(_T("<span class=\"statusDot status%u\" title=\"eD2K\"></span><span class=\"statusDot status%u\" title=\"Kad\"></span>"),
			ed2kState, kadState);
		SetElementHtml(_T("connectedImg"), CComBSTR(statusHtml));
	}
