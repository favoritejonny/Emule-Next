...
static CString CreateSecureWebSessionToken()
{
	unsigned int auRandom[4];
	if (BCryptGenRandom(NULL, reinterpret_cast<PUCHAR>(auRandom), sizeof(auRandom), BCRYPT_USE_SYSTEM_PREFERRED_RNG) != 0)
		return CString();

	CString sToken;
	for (size_t i = 0; i < _countof(auRandom); ++i)
		sToken.AppendFormat(_T("%08x"), auRandom[i]);
	return sToken;
}

static uint64 ParseSearchSizeInBytes(const CString &sValue)
{
	static const uint64 BYTES_PER_MEBIBYTE = 1024ui64 * 1024ui64;
	const __int64 nMegabytes = _tstoi64(sValue);
	if (nMegabytes <= 0)
		return 0;

	const uint64 uMegabytes = static_cast<uint64>(nMegabytes);
	const uint64 uMaxMegabytes = (std::numeric_limits<uint64>::max)() / BYTES_PER_MEBIBYTE;
	return (uMegabytes > uMaxMegabytes ? uMaxMegabytes : uMegabytes) * BYTES_PER_MEBIBYTE;
}

static bool GetGzipBufferSize(int nSourceLength, uLongf &rDestinationLength)
{
	if (nSourceLength < 0)
		return false;

	const uint64 uSourceLength = static_cast<uint64>(nSourceLength);
	const uint64 uDestinationLength = uSourceLength + (uSourceLength >> 12) + (uSourceLength >> 14)
		+ (uSourceLength >> 25) + 13 + 18;
	if (uDestinationLength > static_cast<uint64>((std::numeric_limits<uLongf>::max)()))
		return false;

	rDestinationLength = static_cast<uLongf>(uDestinationLength);
	return true;
}
...
{
	CSingleLock lock(&m_csSessionData, TRUE);
...
void CWebServer::_ProcessURL(const ThreadData &Data)
{
	...
	try {
		...
		{
			CSingleLock lock(&pThis->m_csSessionData, TRUE);
			for (INT_PTR i = pThis->m_Params.badlogins.GetCount(); --i >= 0;)
				if (HasTickCountElapsed(curTick, pThis->m_Params.badlogins[i].timestamp, MIN2MS(15)))
		...
		if (_ParseURL(Data.sURL, _T("w")) == _T("password")) {
			...
			if (MD5Sum(_ParseURL(Data.sURL, _T("p"))).GetHashString() == thePrefs.GetWSPass()) {
				bool bSessionReady = justAddLink;
				if (!justAddLink) {
					// user wants to login
					CString sNewSession(CreateSecureWebSessionToken());
					if (!sNewSession.IsEmpty()) {
						Session ses;
						ses.admin = true;
						ses.startTime = CTime::GetCurrentTime();
						ses.sSession = sSession = sNewSession;
				...
				if (bSessionReady) {
					...
					LogWarning(LOG_STATUSBAR, _T("Unable to create a secure WebServer session."));
		...
		if (_ParseURL(Data.sURL, _T("w")) == _T("logout"))
			_RemoveSession(Data, sSession);

		std::unique_ptr<Bytef[]> gzipOut;
		DWORD gzipLen = 0;
		CString Out;
		if (_IsLoggedIn(Data, sSession)) {
...
#define SENDFILEBUFSIZE 2048
			...
			if (isUseGzip) {
				...
				try {
					...
					uLongf destLen = 0;
					if (GetGzipBufferSize(strA.GetLength(), destLen)) {
						gzipOut.reset(new Bytef[static_cast<size_t>(destLen)]);
						if (_GzipCompress(gzipOut.get(), &destLen, reinterpret_cast<const Bytef*>(static_cast<LPCSTR>(strA)), static_cast<uLong>(strA.GetLength()), Z_DEFAULT_COMPRESSION) == Z_OK
							&& destLen <= (std::numeric_limits<DWORD>::max)()) {
							bOk = true;
							gzipLen = static_cast<DWORD>(destLen);
						}
					}
				} catch (...) {
					ASSERT(0);
		...
		if (!isUseGzip)
			...
			Data.pSocket->SendContent(HTTPInitGZ, gzipOut.get(), gzipLen);
...
CString CWebServer::_GetHeader(const ThreadData &Data, const CString &sSession)
{
	...
	swCommand.AppendFormat(_T("&amp;cat=%s&amp;dummy=%lu"), (LPCTSTR)_ParseURL(Data.sURL, _T("cat")), ::GetTickCount());
	...
	if (thePrefs.GetMaxDownload() == UNLIMITED)
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%.0f"), theApp.downloadqueue->GetDatarate() / 10.24 / thePrefs.GetMaxGraphDownloadRate());
	else
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%.0f"), theApp.downloadqueue->GetDatarate() / 10.24 / thePrefs.GetMaxDownload());
	Out.Replace(_T("[DownloadValue]"), HTTPHeader);

	if (thePrefs.GetMaxUpload() == UNLIMITED)
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%.0f"), theApp.uploadqueue->GetDatarate() / 10.24 / thePrefs.GetMaxGraphUploadRate(true));
	else
		_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%.0f"), theApp.uploadqueue->GetDatarate() / 10.24 / thePrefs.GetMaxUpload());
	Out.Replace(_T("[UploadValue]"), HTTPHeader);

	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%.0f"), (100.0 * theApp.listensocket->GetOpenSockets()) / thePrefs.GetMaxConnections());
	Out.Replace(_T("[ConnectionValue]"), HTTPHeader);
	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%.1f"), theApp.uploadqueue->GetDatarate() / 1024.0);
	Out.Replace(_T("[CurUpload]"), HTTPHeader);
	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%.1f"), theApp.downloadqueue->GetDatarate() / 1024.0);
	Out.Replace(_T("[CurDownload]"), HTTPHeader);
	_sntprintf_s(HTTPHeader, _countof(HTTPHeader), _TRUNCATE, _T("%u.0"), theApp.listensocket->GetOpenSockets());
...
CString CWebServer::_GetServerList(const ThreadData &Data)
{
	...
	if (bAdmin) {
		...
		if (iMenu >= 0 && iMenu < static_cast<int>(_countof(WSserverColumnHidden))) {
	...
	if (ServerArray.GetCount() > 1) {
		std::sort(ServerArray.GetData(), ServerArray.GetData() + ServerArray.GetCount(), [&prm](const ServerEntry &left, const ServerEntry &right) {
			return _ServerCmp(&prm, &left, &right) < 0;
		});
	}
...
CString CWebServer::_GetTransferList(const ThreadData &Data)
{
	...
	if (catp.IsEmpty())
		cat = _GetLastUserCat(Data, sSession);
	else {
		cat = _tstoi(catp);
		_SetLastUserCat(Data, sSession, cat);
	}
	// commands
	CString sCat;
	if (cat != 0)
	...
	if (HTTPTemp == _T("menudown")) {
		int iMenu = _tstol(_ParseURL(Data.sURL, _T("m")));
		if (iMenu >= 0 && iMenu < static_cast<int>(_countof(WSdownloadColumnHidden))) {
		...
		int iMenu = _tstol(_ParseURL(Data.sURL, _T("m")));
		if (iMenu >= 0 && iMenu < static_cast<int>(_countof(WSuploadColumnHidden))) {
		...
		int iMenu = _tstol(_ParseURL(Data.sURL, _T("m")));
		if (iMenu >= 0 && iMenu < static_cast<int>(_countof(WSqueueColumnHidden))) {
	...
	if (FilesArray.GetCount() > 1) {
		std::sort(FilesArray.GetData(), FilesArray.GetData() + FilesArray.GetCount(), [&dprm](const DownloadFiles &left, const DownloadFiles &right) {
			return _DownloadCmp(&dprm, &left, &right) < 0;
		});
	}
	...
	if (UploadArray.GetCount() > 1) {
		std::sort(UploadArray.GetData(), UploadArray.GetData() + UploadArray.GetCount(), [&uprm](const UploadUsers &left, const UploadUsers &right) {
			return _UploadCmp(&uprm, &left, &right) < 0;
		});
	}
...
void CWebServer::_MakeTransferList(CString &Out, CWebServer *pThis, const ThreadData &Data, CArray<DownloadFiles> &filesArray, CArray<UploadUsers> &uploadArray, bool bAdmin)
{
	// Displaying
	int nCountQueue = 0;
	...
	HTTPTemp.Format(_T("%s: %i"), (LPCTSTR)GetResString(IDS_SF_FILE), static_cast<int>(filesArray.GetCount()));
			...
			if (QueueArray[i].sClientExtra == _T("none")) {
				...
				if (WSqueueColumnHidden[3])
					*HTTPTempC = _T('\0');
					...
					_sntprintf_s(HTTPTempC, _countof(HTTPTempC), _TRUNCATE, _T("%i"), QueueArray[i].nScore);
...
CString CWebServer::_GetSharedFilesList(const ThreadData &Data)
{
	...
	if (_ParseURL(Data.sURL, _T("c")) == _T("menu")) {
		...
		if (iMenu >= 0 && iMenu < static_cast<int>(_countof(WSsharedColumnHidden))) {
	...
	if (SharedArray.GetCount() > 1) {
		std::sort(SharedArray.GetData(), SharedArray.GetData() + SharedArray.GetCount(), [&prm](const SharedFiles &left, const SharedFiles &right) {
			return _SharedCmp(&prm, &left, &right) < 0;
		});
	}
	...
	for (INT_PTR i = 0; i < SharedArray.GetCount(); ++i) {
		...
		if (WSsharedColumnHidden[1])
			*HTTPTempC = _T('\0');
			...
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), _TRUNCATE, _T(" (%s)"), (LPCTSTR)CastItoXBytes(SharedArray[i].nFileAllTimeTransferred));
		HTTPProcessData.Replace(_T("[FileAllTimeTransferred]"), HTTPTempC);
		//2
		if (WSsharedColumnHidden[2])
			*HTTPTempC = _T('\0');
		else
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), _TRUNCATE, _T("%i"), SharedArray[i].nFileRequests);
		HTTPProcessData.Replace(_T("[FileRequests]"), HTTPTempC);
		if (!WSsharedColumnHidden[2])
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), _TRUNCATE, _T(" (%i)"), SharedArray[i].nFileAllTimeRequests);
		HTTPProcessData.Replace(_T("[FileAllTimeRequests]"), HTTPTempC);
		//3
		if (WSsharedColumnHidden[3])
			*HTTPTempC = _T('\0');
		else
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), _TRUNCATE, _T("%i"), SharedArray[i].nFileAccepts);
		HTTPProcessData.Replace(_T("[FileAccepts]"), HTTPTempC);
		if (!WSsharedColumnHidden[3])
			_sntprintf_s(HTTPTempC, _countof(HTTPTempC), _TRUNCATE, _T(" (%i)"), SharedArray[i].nFileAllTimeAccepts);
...
CString CWebServer::_GetPreferences(const ThreadData &Data)
{
	...
	if ((_ParseURL(Data.sURL, _T("saveprefs")) == _T("true")) && _IsSessionAdmin(Data, sSession)) {
		...
		if (!_ParseURL(Data.sURL, _T("refresh")).IsEmpty()) {
			int nRefresh = _tstoi(_ParseURL(Data.sURL, _T("refresh")));
			if (nRefresh >= 0)
				thePrefs.SetWebPageRefresh(nRefresh);
		}

		strTmp = _ParseURL(Data.sURL, _T("maxcapdown"));
		if (!strTmp.IsEmpty()) {
			int nCapacity = _tstoi(strTmp);
			if (nCapacity >= 0)
				thePrefs.SetMaxGraphDownloadRate(static_cast<uint32>(nCapacity));
		}
		strTmp = _ParseURL(Data.sURL, _T("maxcapup"));
		if (!strTmp.IsEmpty()) {
			int nCapacity = _tstoi(strTmp);
			if (nCapacity >= 0)
				thePrefs.SetMaxGraphUploadRate(static_cast<uint32>(nCapacity));
		}

		strTmp = _ParseURL(Data.sURL, _T("maxdown"));
		if (!strTmp.IsEmpty()) {
			int nSpeed = _tstoi(strTmp);
			if (nSpeed >= 0)
				thePrefs.SetMaxDownload(nSpeed > 0 ? static_cast<uint32>(nSpeed) : UNLIMITED);
		}
		strTmp = _ParseURL(Data.sURL, _T("maxup"));
		if (!strTmp.IsEmpty()) {
			int nSpeed = _tstoi(strTmp);
			if (nSpeed >= 0)
				thePrefs.SetMaxUpload(nSpeed > 0 ? static_cast<uint32>(nSpeed) : UNLIMITED);
		}

		if (!_ParseURL(Data.sURL, _T("maxsources")).IsEmpty()) {
			int nSources = _tstoi(_ParseURL(Data.sURL, _T("maxsources")));
			if (nSources >= 0)
				thePrefs.SetMaxSourcesPerFile(static_cast<UINT>(nSources));
		}
		if (!_ParseURL(Data.sURL, _T("maxconnections")).IsEmpty()) {
			int nConnections = _tstoi(_ParseURL(Data.sURL, _T("maxconnections")));
			if (nConnections >= 0)
				thePrefs.SetMaxConnections(static_cast<UINT>(nConnections));
		}
		if (!_ParseURL(Data.sURL, _T("maxconnectionsperfive")).IsEmpty()) {
			int nConnections = _tstoi(_ParseURL(Data.sURL, _T("maxconnectionsperfive")));
			if (nConnections >= 0)
				thePrefs.SetMaxConsPerFive(static_cast<UINT>(nConnections));
		}
	}
...
{
	static const Bytef GZIP_HEADER[] = {0x1f, 0x8b, Z_DEFLATED, 0, 0, 0, 0, 0, 0, 255};
	if (dest == NULL || destLen == NULL || (source == NULL && sourceLen != 0))
		return Z_STREAM_ERROR;
	if (*destLen < sizeof(GZIP_HEADER) + sizeof(uint32) * 2
		|| *destLen - sizeof(GZIP_HEADER) - sizeof(uint32) * 2 > (std::numeric_limits<uInt>::max)()
		|| sourceLen > (std::numeric_limits<uInt>::max)())
		return Z_BUF_ERROR;
	...
	stream.next_out = dest + sizeof(GZIP_HEADER);
	stream.avail_out = static_cast<uInt>(*destLen - sizeof(GZIP_HEADER) - sizeof(uint32) * 2);
	// do it
	...
	crc = crc32(crc, source, static_cast<uInt>(sourceLen));
	const uint32 uCRC = static_cast<uint32>(crc);
	const uint32 uSourceLen = static_cast<uint32>(sourceLen);
	Bytef *pTrailer = dest + sizeof(GZIP_HEADER) + stream.total_out;
	memcpy(pTrailer, &uCRC, sizeof(uCRC));
	memcpy(pTrailer + sizeof(uCRC), &uSourceLen, sizeof(uSourceLen));
	*destLen = sizeof(GZIP_HEADER) + stream.total_out + sizeof(uCRC) + sizeof(uSourceLen);

	return Z_OK;
}

bool CWebServer::_IsLoggedIn(const ThreadData &Data, const CString &sSession)
{
	CWebServer *pThis = reinterpret_cast<CWebServer*>(Data.pThis);
	if (pThis == NULL)
		return false;
	...
	if (!sSession.IsEmpty())
		for (INT_PTR i = pThis->m_Params.Sessions.GetCount(); --i >= 0;)
			if (pThis->m_Params.Sessions[i].sSession == sSession) {
...
bool CWebServer::_RemoveSession(const ThreadData &Data, const CString &sSession)
{
	CWebServer *pThis = reinterpret_cast<CWebServer*>(Data.pThis);
	if (pThis == NULL || sSession.IsEmpty())
		return false;
	...
	if (bRemoved) {
		AddLogLine(true, (LPCTSTR)GetResString(IDS_WEB_SESSIONEND), (LPCTSTR)ipstr(Data.inadr));
		SendMessage(theApp.emuledlg->m_hWnd, WEB_GUI_INTERACTION, WEBGUIIA_UPDATEMYINFO, 0);
	}
	return bRemoved;
}

Session CWebServer::_GetSessionByID(const ThreadData &Data, const CString &sSession)
{
	CWebServer *pThis = reinterpret_cast<CWebServer*>(Data.pThis);
	if (pThis == NULL)
		return Session{};
...
bool CWebServer::_IsSessionAdmin(const ThreadData &Data, const CString &sSession)
{
	CWebServer *pThis = reinterpret_cast<CWebServer*>(Data.pThis);
	if (pThis == NULL)
		return false;
...
CString CWebServer::_GetSearch(const ThreadData &Data)
{
	...
	if (_ParseURL(Data.sURL, _T("c")) == _T("menu")) {
		int iMenu = _tstoi(_ParseURL(Data.sURL, _T("m")));
		bool bValue = _tstoi(_ParseURL(Data.sURL, _T("v"))) != 0;
		if (iMenu >= 0 && iMenu < static_cast<int>(_countof(WSsearchColumnHidden))) {
	...
	if (!_ParseURL(Data.sURL, _T("tosearch")).IsEmpty() && bSessionAdmin) {
		...
		pParams->ullMinSize = ParseSearchSizeInBytes(_ParseURL(Data.sURL, _T("min")));
		pParams->ullMaxSize = ParseSearchSizeInBytes(_ParseURL(Data.sURL, _T("max")));
		if (pParams->ullMaxSize < pParams->ullMinSize)
			pParams->ullMaxSize = 0;

		const CString &s(_ParseURL(Data.sURL, _T("avail")));
		const int nAvailability = s.IsEmpty() ? 0 : _tstoi(s);
		pParams->uAvailability = nAvailability > 0 ? static_cast<uint32>(nAvailability) : 0;
	...
	if (!sSort.IsEmpty()) {
		int iSortBy = _tstoi(sSort);
		if (iSortBy >= 0 && iSortBy <= 4)
			pThis->m_iSearchSortby = iSortBy;
	}
...
INT_PTR CWebServer::UpdateSessionCount()
{
	...
	INT_PTR nSessionCount;
	{
		...
		if (thePrefs.GetWebTimeoutMins() > 0) {
			...
			bChanged = oldvalue != m_Params.Sessions.GetCount();
		}
...
void CWebServer::_SetLastUserCat(const ThreadData &Data, const CString &sSession, int cat)
{
	CWebServer *pThis = reinterpret_cast<CWebServer*>(Data.pThis);
	if (pThis == NULL)
		return;
...
int CWebServer::_GetLastUserCat(const ThreadData &Data, const CString &sSession)
{
	CWebServer *pThis = reinterpret_cast<CWebServer*>(Data.pThis);
	if (pThis == NULL)
		return 0;
...
void CWebServer::_ProcessFileReq(const ThreadData &Data)
{
	...
	while (!filename.IsEmpty() && filename[0] == _T('\\'))
		filename.Delete(0, 1);
	if (filename.IsEmpty() || filename.Find(_T("..")) >= 0 || filename.Find(_T(':')) >= 0) {
		Data.pSocket->SendReply("HTTP/1.1 404 File not found\r\n");
		return;
	}
	...
	if (file.Open(filename, CFile::modeRead | CFile::shareDenyWrite | CFile::typeBinary)) {
		...
		const uint64 maxUploadSize = static_cast<uint64>(thePrefs.GetMaxWebUploadFileSizeMB()) * 1024 * 1024;
		if ((thePrefs.GetMaxWebUploadFileSizeMB() != 0 && fileSize > maxUploadSize) || fileSize > UINT_MAX) {
			file.Close();
			Data.pSocket->SendReply("HTTP/1.1 403 Forbidden\r\n");
			return;
		}

		const UINT fileSize32 = static_cast<UINT>(fileSize);
		if (fileSize32 == 0) {
			file.Close();
			Data.pSocket->SendContent((CStringA)contenttype, "", 0);
			return;
		}

		std::unique_ptr<char[]> buffer;
		try {
			buffer.reset(new char[fileSize32]);
		} catch (...) {
			file.Close();
		...
		const UINT size = file.Read(buffer.get(), fileSize32);
		file.Close();
		Data.pSocket->SendContent((CStringA)contenttype, buffer.get(), size);
	} else
		Data.pSocket->SendReply("HTTP/1.1 404 File not found\r\n");
}
