...
BOOL CemuleApp::InitInstance()
{
	...
	//
	// A local dump is offered after a crash. It is never uploaded automatically.
	theCrashDumper.uCreateCrashDump = GetProfileInt(EMULE_NEXT_PRODUCT_NAME, _T("CreateCrashDump"), 1);
#if !defined(_BETA) && !defined(_DEVBUILD)
	if (theCrashDumper.uCreateCrashDump > 0)
#endif
		theCrashDumper.Enable(GetProductDisplayName(), true, sConfDir);
	...
	Log(_T("Starting %s (eD2K protocol v%s)"), GetProductDisplayName(), (LPCTSTR)m_strCurVersionLong);
...
bool CemuleApp::CopyTextToClipboard(const CString &strText)
{
	...
	const SIZE_T uTextSize = (static_cast<SIZE_T>(strText.GetLength()) + 1) * sizeof(TCHAR);
	HGLOBAL hGlobalT = ::GlobalAlloc(GHND | GMEM_SHARE, uTextSize);
	if (hGlobalT != NULL) {
		LPTSTR pGlobalT = static_cast<LPTSTR>(::GlobalLock(hGlobalT));
		if (pGlobalT != NULL) {
			memcpy(pGlobalT, static_cast<LPCTSTR>(strText), uTextSize);
	...
	const SIZE_T uAnsiTextSize = static_cast<SIZE_T>(strTextA.GetLength()) + 1;
	HGLOBAL hGlobalA = ::GlobalAlloc(GHND | GMEM_SHARE, uAnsiTextSize);
	if (hGlobalA != NULL) {
		LPSTR pGlobalA = static_cast<LPSTR>(::GlobalLock(hGlobalA));
		if (pGlobalA != NULL) {
			memcpy(pGlobalA, static_cast<LPCSTR>(strTextA), uAnsiTextSize);
...
void CemuleApp::ShowHelp(UINT uTopic, UINT uCmd)
{
	...
	if (!ShowWebHelp(uTopic) && GetLangHelpFilePath(strHelpFilePath)) {
		SetHelpFilePath(strHelpFilePath);
		WinHelpInternal(uTopic, uCmd);
	}
}

static UINT GetOfficialHelpLanguage(LANGID languageID)
{
	// Language numbers used by the official eMule help site. Keep only exact
	// matches: client languages not published there fall back to English.
	switch (languageID) {
		case LANGID_DE_DE:  return 2;
		case LANGID_FR_FR:  return 13;
		case LANGID_HE_IL:  return 14;
		case LANGID_ZH_TW:  return 16;
		case LANGID_ES_ES_T: return 17;
		case LANGID_IT_IT:  return 18;
		case LANGID_PL_PL:  return 20;
		case LANGID_LT_LT:  return 22;
		case LANGID_ET_EE:  return 25;
		case LANGID_DA_DK:  return 26;
		case LANGID_SV_SE:  return 27;
		case LANGID_NL_NL:  return 29;
		case LANGID_PT_BR:  return 30;
		case LANGID_CZ_CZ:  return 32;
		case LANGID_PT_PT:  return 33;
		case LANGID_RU_RU:  return 34;
		case LANGID_CA_ES:  return 35;
		case LANGID_TR_TR:  return 36;
		case LANGID_FI_FI:  return 37;
		case LANGID_RO_RO:  return 38;
		case LANGID_EL_GR:  return 39;
		case LANGID_ZH_CN:  return 42;
		case LANGID_EN_US:
		default:            return 1;
	}
}
...
{
	const UINT uHelpLanguage = GetOfficialHelpLanguage(thePrefs.GetLanguageID());
	CString strHelpURL;

	// The main Help command opens the selected official eMule guide topic.
	if (uTopic == 0) {
		strHelpURL.Format(EMULE_NEXT_HELP_TOPIC_URL_FORMAT, uHelpLanguage);
	} else {
		// Context Help always opens the official guide index in the language
		// selected in eMule Next. This avoids mixing the official site with the
		// obsolete bundled eMule Next HTML guide.
		strHelpURL.Format(EMULE_NEXT_TRANSFERS_HELP_URL_FORMAT, uHelpLanguage);
	}
...
int CemuleApp::GetFileTypeSystemImageIdx(LPCTSTR pszFilePath, int iLength /* = -1 */, bool bNormalsSize)
{
	...
	int iconIndex;
	if (bNormalsSize) {
		if (!m_aBigExtToSysImgIdx.Lookup(pszCacheExt, iconIndex)) {
	...
	} else if (!m_aExtToSysImgIdx.Lookup(pszCacheExt, iconIndex)) {
...
HICON CemuleApp::LoadIcon(LPCTSTR lpszResourceName, int cx, int cy, UINT uFlags) const
{
	...
	// eMule Next provides scalable, alpha-enabled artwork for the interface
	// icons it owns. A skin-supplied icon still takes precedence above.
	if (hIcon == NULL && CreateNextModernIcon(lpszResourceName, cx, cy, &hIcon))
		return hIcon;
