	...
	if (readFile != NULL) {
		...
		while (!feof(readFile)) {
			...
			if (!strTitle.Trim().IsEmpty()) {
				...
				if (pszCommandArgs) {
					strCommandArgs = pszCommandArgs;
					strCommand.Truncate(static_cast<int>(pszCommandArgs - (LPCTSTR)strCommand));
				}
				...
				svc.strCommandArgs = strCommandArgs.Trim();
...
void ExecutePartFile(CPartFile *file, LPCTSTR pszCommand, LPCTSTR pszCommandArgs)
{
	...
	const INT_PTR nShellResult = reinterpret_cast<INT_PTR>(ShellExecute(NULL, pszVerb, strCommand, strArgs.IsEmpty() ? NULL : (LPCTSTR)strArgs, strCommandDir.IsEmpty() ? NULL : (LPCTSTR)strCommandDir, SW_SHOWNORMAL));
	if (nShellResult <= 32) {
		//
		...
		LPCTSTR strSysErrMsg = GetShellExecuteErrMsg(static_cast<DWORD>(nShellResult));
		if (*strSysErrMsg)
