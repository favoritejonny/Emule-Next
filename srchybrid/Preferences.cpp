...
namespace
{
	UINT GetAutomaticMinFreeDiskSpace(LPCTSTR pszTempDirectory)
	{
		const uint64 uMinimumReserve = 512ui64 * 1024 * 1024;
		const uint64 uMaximumReserve = 2ui64 * 1024 * 1024 * 1024;

		ULARGE_INTEGER uliFreeBytesAvailable;
		ULARGE_INTEGER uliTotalBytes;
		ULARGE_INTEGER uliTotalFreeBytes;
		if (!::GetDiskFreeSpaceEx(pszTempDirectory, &uliFreeBytesAvailable, &uliTotalBytes, &uliTotalFreeBytes))
			return static_cast<UINT>(uMinimumReserve);

		uint64 uReserve = uliTotalBytes.QuadPart / 100;
		if (uReserve < uMinimumReserve)
			uReserve = uMinimumReserve;
		else if (uReserve > uMaximumReserve)
			uReserve = uMaximumReserve;
		return static_cast<UINT>(uReserve);
	}
}
...
bool CPreferences::IsPortableMode()
{
	// The marker is deliberately a file rather than a registry or command-line
	// setting, so a copied portable package keeps its behaviour on another PC.
	static bool bDetected = false;
	static bool bPortable = false;
	if (!bDetected) {
		bDetected = true;
		TCHAR szModulePath[MAX_PATH];
		DWORD dwModulePathLen = ::GetModuleFileName(NULL, szModulePath, _countof(szModulePath));
		if (dwModulePathLen != 0 && dwModulePathLen < _countof(szModulePath)) {
			LPTSTR pszFileName = _tcsrchr(szModulePath, _T('\\'));
			if (pszFileName != NULL) {
				pszFileName[1] = _T('\0');
				bPortable = ::PathFileExists(CString(szModulePath) + _T("eMuleNext.portable")) != FALSE;
			}
		}
	}
	return bPortable;
...
void CPreferences::SaveStats(int bBackUp)
{
	...
	if (theApp.serverconnect->IsConnected())
		ini.WriteInt(_T("ConnAvgConnections"), (UINT)((theApp.listensocket->GetAverageConnections() + static_cast<float>(cumConnAvgConnections)) / 2.0f));
...
void CPreferences::SavePreferences()
{
	...
	ini.WriteDWORD(_T("MinFreeDiskSpace"), m_uMinFreeDiskSpace);
...
void CPreferences::LoadPreferences()
{
	...
	// Automatic checks remain disabled until eMule Next has a signed update
	// feed. The manual update command continues to open the project page.
	updatenotify = false;
	...
	m_uDeadServerRetries = ini.GetInt(_T("DeadServerRetry"), 3);
	...
	m_bAutoStart = IsPortableMode() ? false : ini.GetBool(_T("AutoStart"), false);
	...
	m_uMinFreeDiskSpace = ini.GetDWORD(_T("MinFreeDiskSpace"), GetAutomaticMinFreeDiskSpace(GetTempDir()));
	m_bSparsePartFiles = ini.GetBool(_T("SparsePartFiles"), VolumeSupportsSparseFiles(GetTempDir()));
	...
	autotakeed2klinks = IsPortableMode() ? false : ini.GetBool(_T("AutoTakeED2KLinks"), true);
	...
	if (m_bEnableVerboseOptions) {
		...
		m_bLogRatingDescReceived = ini.GetBool(_T("LogRatingDescReceived"), false);
		m_bLogSecureIdent = ini.GetBool(_T("LogSecureIdent"), false);
		...
		m_bLogUlDlEvents = ini.GetBool(_T("LogUlDlEvents"), false);
	} else {
	...
	const int minFileBufferSize = 16 * 1024;
	const int maxFileBufferSize = (1024 + 512) * 1024;
	const int legacyFileBufferSize = ini.GetInt(_T("FileBufferSizePref"), 0);
	int defaultFileBufferSize = 256 * 1024;
	if (legacyFileBufferSize > 0) {
		const ULONGLONG convertedLegacyFileBufferSize = ((static_cast<ULONGLONG>(legacyFileBufferSize) * 15000 + 512) / 1024) * 1024;
		defaultFileBufferSize = convertedLegacyFileBufferSize > static_cast<ULONGLONG>(maxFileBufferSize)
			? maxFileBufferSize
			: static_cast<int>(convertedLegacyFileBufferSize);
	}
	const int configuredFileBufferSize = ini.GetInt(_T("FileBufferSize"), defaultFileBufferSize);
	m_uFileBufferSize = static_cast<UINT>(configuredFileBufferSize < minFileBufferSize
		? minFileBufferSize
		: configuredFileBufferSize > maxFileBufferSize
			? maxFileBufferSize
			: configuredFileBufferSize);
	const int configuredFileBufferTimeLimit = ini.GetInt(_T("FileBufferTimeLimit"), 60);
	const DWORD fileBufferTimeLimitSeconds = static_cast<DWORD>(configuredFileBufferTimeLimit > 0 ? configuredFileBufferTimeLimit : 60);
	const DWORD maxFileBufferTimeLimitSeconds = 0xFFFFFFFFUL / 1000UL;
	m_uFileBufferTimeLimit = fileBufferTimeLimitSeconds > maxFileBufferTimeLimitSeconds
		? 0xFFFFFFFFUL
		: fileBufferTimeLimitSeconds * 1000UL;
	...
	// Match the supported range of the Preferences slider, even for manually edited INI files.
	m_iQueueSize = max(static_cast<INT_PTR>(2000), min(static_cast<INT_PTR>(10000), m_iQueueSize));
	...
	m_iExtractMetaData = ini.GetInt(_T("ExtractMetaData"), 0); // 0=disable, 1=mp3, 2=MediaDet
	...
	m_byLogLevel = ini.GetInt(_T("DebugLogLevel"), DLP_DEFAULT);
	...
	// The docked command bar is a legacy download-only control. Keep it
	// available from the download context menu, but do not show it by default.
	// Existing profiles are migrated once so the cleaner layout is applied to
	// current eMule Next installations as well.
	const bool bLegacyDownloadToolbarReviewed = ini.GetBool(_T("eMuleNextLegacyDownloadToolbarReviewed"), false);
	m_bShowDownloadToolbar = bLegacyDownloadToolbarReviewed
		? ini.GetBool(_T("ShowDownloadToolbar"), false)
		: false;
	if (!bLegacyDownloadToolbarReviewed)
		ini.WriteBool(_T("eMuleNextLegacyDownloadToolbarReviewed"), true);
...
UINT CPreferences::GetDefaultMaxHalfConnections()
{
	return GetWindowsVersion() >= _WINVER_7_ ? 20 : 9;
}
...
{
// Migrate the previous eMule Next product alias to the current repository
// address. A deliberately chosen nickname is always kept unchanged.
	if (strCheck == EMULE_NEXT_LEGACY_DEFAULT_ALIAS)
		return true;
...
bool CPreferences::GetSparsePartFiles(LPCTSTR pszPath)
{
	...
	if (!m_bSparsePartFiles || GetWindowsVersion() == _WINVER_VISTA_)
		return false;

	if (pszPath != NULL)
		return VolumeSupportsSparseFiles(pszPath);

	for (INT_PTR i = 0; i < GetTempDirCount(); ++i) {
		if (VolumeSupportsSparseFiles(GetTempDir(i)))
			return true;
	}
	return false;
}
