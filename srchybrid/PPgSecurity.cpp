...
void CPPgSecurity::DoDataExchange(CDataExchange *pDX)
{
	...
	DDX_Control(pDX, IDC_RELOADFILTER, m_reloadIPFilterButton);
	DDX_Control(pDX, IDC_EDITFILTER, m_editIPFilterButton);
}
...
{
	if (!PathFileExists(CIPFilter::GetDefaultFilePath())) {
...
{
	const CString strFilePath(CIPFilter::GetDefaultFilePath());
	if (!PathFileExists(strFilePath)) {
		FILE *writeFile = _tfsopen(strFilePath, _T("wt"), _SH_DENYWR);
		if (writeFile == NULL) {
			AfxMessageBox(GetResString(IDS_ERROR_SAVEFILE), MB_OK | MB_ICONERROR);
			return;
		}
		const bool bWriteFailed = fputs("# eMule Next IP filter\n", writeFile) == EOF
			|| fputs("# Format: start IP - end IP , level , description\n", writeFile) == EOF
			|| fputs("# Example: 001.002.003.004 - 001.002.003.255 , 100 , Example range\n", writeFile) == EOF;
		if (fclose(writeFile) != 0 || bWriteFailed) {
			AfxMessageBox(GetResString(IDS_ERROR_SAVEFILE), MB_OK | MB_ICONERROR);
			return;
		}
	}
	...
	strParameters.Format(_T("\"%s\""), (LPCTSTR)strFilePath);
	HINSTANCE hResult = ::ShellExecute(NULL, _T("open"), thePrefs.GetTxtEditor(), strParameters, NULL, SW_SHOW);
