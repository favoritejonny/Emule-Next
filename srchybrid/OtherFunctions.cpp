...
bool CheckFileOpen(LPCTSTR pszFilePath, LPCTSTR pszFileTitle)
{
	...
	UINT uWarning = 0;
	if (_tcsicmp(::PathFindExtension(pszFilePath), _T(".iso")) == 0)
		uWarning = IDS_FILE_WARNING_ISO;
	else if (GetDRM(pszFilePath))
		uWarning = IDS_FILE_WARNING_DRM;

	if (uWarning != 0) {
		CString strWarning;
		strWarning.Format(GetResString(uWarning), pszFileTitle != NULL ? pszFileTitle : pszFilePath);
...
{
	// The legacy "run as unprivileged user" option starts eMule under the
	// separate eMule_Secure Windows account. That account cannot activate the
	// interactive user's browser. In that case provide a visible, safe fallback
	// instead of reporting success while apparently doing nothing.
	TCHAR szUserName[256] = {};
	DWORD cchUserName = _countof(szUserName);
	bool bRestrictedToken = false;
	HANDLE hProcessToken = NULL;
	if (::OpenProcessToken(::GetCurrentProcess(), TOKEN_QUERY, &hProcessToken)) {
		bRestrictedToken = ::IsTokenRestricted(hProcessToken) != FALSE;
		::CloseHandle(hProcessToken);
	}
	if (lpURL != NULL && ::PathIsURL(lpURL)
		&& (bRestrictedToken
			|| (::GetUserName(szUserName, &cchUserName)
				&& CString(szUserName).CompareNoCase(_T("eMule_Secure")) == 0)))
	{
		theApp.CopyTextToClipboard(lpURL);
		CString strMessage;
		strMessage.Format(_T("%s:\r\n\r\n%s"),
			static_cast<LPCTSTR>(GetResString(IDS_COPY)), lpURL);
...
bool Ask4RegFix(bool checkOnly, bool dontAsk, bool bAutoTakeCollections)
{
	...
	if (!bGlobalSet) {
		...
		if (dontAsk || (LocMessageBox(IDS_ASSIGNED2K, MB_ICONQUESTION | MB_YESNO, 0) == IDYES)) {
			// Preserve any previous handler only after the user has approved
			// the change. This avoids creating empty association keys when the
			...
			if (regkey.Create(hkeyCR, _T("Software\\Classes\\ed2k\\shell\\open\\command")) == ERROR_SUCCESS) {
				...
				VERIFY(regkey.SetStringValue(NULL, CString(modbuffer) + _T(",0")) == ERROR_SUCCESS);
	...
	if (bGlobalSet && bAutoTakeCollections) {
		if (checkOnly)
			return DoCollectionRegFix(true);
		if (DoCollectionRegFix(true))
			DoCollectionRegFix(false);
	}

	if (!checkOnly && bGlobalSet)
...
bool VolumeSupportsSparseFiles(LPCTSTR pDirectory)
{
	if (pDirectory == NULL || *pDirectory == _T('\0'))
		return false;

	TCHAR szVolumePath[MAX_PATH];
	if (!::GetVolumePathName(pDirectory, szVolumePath, _countof(szVolumePath)))
		return false;

	DWORD dwFileSystemFlags = 0;
	return ::GetVolumeInformation(szVolumePath, NULL, 0, NULL, NULL, &dwFileSystemFlags, NULL, 0)
		&& (dwFileSystemFlags & FILE_SUPPORTS_SPARSE_FILES) != 0;
}
...
void CWebServices::Edit()
{
	...
	if (!PathFileExists(strFilePath)) {
		CStdioFile file;
		if (!file.Open(strFilePath, CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite | CFile::typeText)) {
			AfxMessageBox(GetResString(IDS_ERROR_SAVEFILE), MB_OK | MB_ICONERROR);
			return;
		}
		file.WriteString(_T("# eMule Next web services\n"));
		file.WriteString(_T("# Format: Menu name, URL\n"));
		file.WriteString(_T("# File macros: #hashid, #filesize, #filename, #name, #cleanfilename, #cleanname\n"));
		file.WriteString(_T("# Example: Project website, https://github.com/favoritejonny/Emule-Next\n"));
		file.Close();
	}

	CString sDat;
	sDat.Format(_T("\"%s\""), (LPCTSTR)strFilePath);
	HINSTANCE hResult = ::ShellExecute(NULL, _T("open"), thePrefs.GetTxtEditor(), sDat, NULL, SW_SHOW);
	if (reinterpret_cast<INT_PTR>(hResult) <= 32)
		AfxMessageBox(GetResString(IDS_ERR_OPEN), MB_OK | MB_ICONERROR);
}
...
ULONGLONG GetDiskFileSize(LPCTSTR pszFilePath)
{
	...
	DWORD dwHigh = 0;
	const DWORD dwLow = ::GetCompressedFileSize(pszFilePath, &dwHigh);
...
{
	if (pszAddress == NULL || iMaxAddress <= 0)
		return;

	const BYTE *pucIP = (BYTE*)&nIP;
	snprintf(pszAddress, static_cast<size_t>(iMaxAddress), "%u.%u.%u.%u", pucIP[0], pucIP[1], pucIP[2], pucIP[3]);
}
...
{
	if (astr.GetCount() > 1) {
		std::sort(astr.GetData(), astr.GetData() + astr.GetCount(), [pfnCompare](const CString &left, const CString &right) {
			return pfnCompare(&left, &right) < 0;
		});
	}
}
...
{
	if (apstr.GetSize() > 1) {
		std::sort(apstr.GetData(), apstr.GetData() + apstr.GetSize(), [pfnCompare](const CString *left, const CString *right) {
			return pfnCompare(&left, &right) < 0;
		});
	}
}
...
bool DoCollectionRegFix(bool checkOnly)
{
	...
	if (bGlobalSet) {
		result = regkey.Open(HKEY_CLASSES_ROOT, COLLECTION_FILEEXTENSION, KEY_READ);
		if (result == ERROR_SUCCESS) {
			TCHAR rbuffer[32];
			ULONG maxsize = _countof(rbuffer);
			if (regkey.QueryStringValue(NULL, rbuffer, &maxsize) != ERROR_SUCCESS
				|| maxsize == 0 || _tcsicmp(rbuffer, _T("eMule")) != 0)
				bGlobalSet = false;
			regkey.Close();
		} else
			bGlobalSet = false;
...
CString MakePath(LPCTSTR drive, LPCTSTR dir, LPCTSTR fname, LPCTSTR ext)
{
	CString path;
	if (drive && *drive) {
		path += drive;
	...
	if (dir && *dir) {
		path += dir;
		if (fname && *fname && path[path.GetLength() - 1] != _T('\\') && path[path.GetLength() - 1] != _T('/'))
			path += _T('\\');
	}
...
CString MakeLongPathCompatible(LPCTSTR filePath)
{
	CString path(filePath ? filePath : _T(""));
	if (path.GetLength() < MAX_PATH || path.IsEmpty())
		return path;

	// Extended syntax is already in use, or this is a device path which must not be rewritten.
	if (path.Left(4) == _T("\\\\?\\") || path.Left(4) == _T("\\\\.\\"))
		return path;

	// Convert absolute UNC and drive-rooted paths only. Relative paths have no valid
	// extended-length representation until their current directory has been resolved.
	if (path.Left(2) == _T("\\\\"))
		return _T("\\\\?\\UNC\\") + path.Mid(2);
	if (path.GetLength() >= 3 && path[1] == _T(':') && (path[2] == _T('\\') || path[2] == _T('/')))
		return _T("\\\\?\\") + path;

	return path;
}

// Wrapper for legacy callers which provide a MAX_PATH-sized output buffer.
// It deliberately avoids _tmakepath_s: recent CRT versions can invoke the invalid
// parameter handler for long components, terminating the process before an error
...
bool _tmakepathlimit(LPTSTR path, LPCTSTR drive, LPCTSTR dir, LPCTSTR fname, LPCTSTR ext)
{
	...
	const CString fullPath = MakePath(drive, dir, fname, ext);
	if (fullPath.GetLength() >= MAX_PATH) {
		path[0] = _T('\0');
		return false;
	}

	const size_t byteCount = (static_cast<size_t>(fullPath.GetLength()) + 1) * sizeof(TCHAR);
	memcpy(path, (LPCTSTR)fullPath, byteCount);
	return true;
}
...
bool HasSubdirectories(const CString &strDir)
{
	...
	for (BOOL bFound = finder.FindFile(sDir + _T('*')); bFound;) {
		...
		const DWORD dwAttributes = ::GetFileAttributes(finder.GetFilePath());
		if (finder.IsDirectory() && !finder.IsDots() && !finder.IsSystem()
			&& dwAttributes != INVALID_FILE_ATTRIBUTES && !(dwAttributes & FILE_ATTRIBUTE_REPARSE_POINT))
			return true;
	}
	return false;
}
