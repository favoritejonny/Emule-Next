...
{
	CList<HTREEITEM, HTREEITEM> pendingItems;
	CArray<HTREEITEM> collapsedItems;
	pendingItems.AddTail(hChild);

	while (!pendingItems.IsEmpty()) {
		HTREEITEM hCurrent = pendingItems.RemoveHead();
		CheckChanged(hCurrent, mark);
		SetCheck(hCurrent, mark);

		const bool bWasExpanded = (GetItemState(hCurrent, TVIS_EXPANDED) & TVIS_EXPANDED) != 0;
		if (!bWasExpanded) {
			Expand(hCurrent, TVE_EXPAND); // make sure child entries are available
			collapsedItems.Add(hCurrent);
		}

		for (HTREEITEM hCurrentChild = GetChildItem(hCurrent); hCurrentChild != NULL; hCurrentChild = GetNextSiblingItem(hCurrentChild))
			pendingItems.AddTail(hCurrentChild);
	}

	for (INT_PTR i = collapsedItems.GetCount(); --i >= 0;)
		Expand(collapsedItems[i], TVE_COLLAPSE); // restore the initial disposition
}
