...
static void FreeFrameGrabMediaType(AM_MEDIA_TYPE &mt)
{
	if (mt.pUnk != NULL) {
		mt.pUnk->Release();
		mt.pUnk = NULL;
...
#define TIMEBETWEENFRAMES	50.0 // could be a param later, if needed
	...
	try {
		...
		if (FAILED(pDet->put_Filename(bstrFilename)))
			return 0;

		bool bFound = false;
		long lStreams = 0;
		if (FAILED(pDet->get_OutputStreams(&lStreams)) || lStreams <= 0)
			return 0;
		for (long i = 0; i < lStreams; ++i) {
			GUID major_type = GUID_NULL;
			if (FAILED(pDet->put_CurrentStream(i)) || FAILED(pDet->get_StreamType(&major_type)))
				continue;
		...
		if (FAILED(hr) || mt.formattype != FORMAT_VideoInfo || mt.pbFormat == NULL || mt.cbFormat < sizeof(VIDEOINFOHEADER)) {
			FreeFrameGrabMediaType(mt);
			return 0;
		}

		VIDEOINFOHEADER vih;
		memcpy(&vih, mt.pbFormat, sizeof vih);
		long width = vih.bmiHeader.biWidth;
		long height = vih.bmiHeader.biHeight;

		// We want absolute height, don't care about orientation.
		if (width <= 0 || height == 0 || height == LONG_MIN) {
			FreeFrameGrabMediaType(mt);
			return 0;
		}
		if (height < 0)
		...
		for (; nFramesGrabbed < nFramesToGrab; ++nFramesGrabbed) {
			...
			if (SUCCEEDED(hr)) {
				if (size <= 0 || size > 64 * 1024 * 1024)
					break;
				const size_t nFrameBitmapLen = sizeof(BITMAPINFOHEADER) + static_cast<size_t>(size);
				...
				BITMAPINFOHEADER sourceBmi;
				memcpy(&sourceBmi, buffer, sizeof sourceBmi);
				if (sourceBmi.biSize < sizeof sourceBmi || sourceBmi.biWidth != width
					|| sourceBmi.biHeight == 0 || sourceBmi.biHeight == LONG_MIN || abs(sourceBmi.biHeight) != height
					|| (sourceBmi.biBitCount != 24 && sourceBmi.biBitCount != 32))
				{
					break;
				}
				...
				if (hFrame == NULL || pBits == NULL) {
					if (hFrame != NULL)
						::DeleteObject(hFrame);
					break;
				}
				HBITMAP hBitmapOld = (HBITMAP)::SelectObject(hdc, hFrame);
				if (hBitmapOld == NULL || hBitmapOld == HGDI_ERROR) {
					::DeleteObject(hFrame);
					break;
				}

				//Resize and reduce colours in one pass without error diffusion.
				const int step0 = sourceBmi.biBitCount / 8;
				const int step1 = bmih.biBitCount / 8;
				const int stride0 = (((width * sourceBmi.biBitCount + 31) & ~31) >> 3);
				const int stride1 = (((bmih.biWidth * bmih.biBitCount + 31) & ~31) >> 3);
					...
					for (int j = 0; j < bmih.biWidth; ++j) {
						...
						if (bReduceColor) {
							if (idx < 0 || memcmp(&last, &q[k], sizeof(BGR))) {
								memcpy(&last, &q[k], sizeof last);
								idx = bestclr(((BITMAPINFO*)bmi)->bmiColors, last);
							}
							ASSERT(idx >= 0 && (DWORD)idx < bmih.biClrUsed);
							*d++ = (byte)idx;
						} else {
							memcpy(d, &q[k], sizeof(BGR));
							if (step1 > static_cast<int>(sizeof(BGR)))
								d[sizeof(BGR)] = 0;
							d += step1;
						}
					}
