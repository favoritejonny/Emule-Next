...
void CDownloadQueue::Process()
{
	...
	while (!average_dr_hist.IsEmpty() && HasTickCountElapsed(curTick, average_dr_hist.Head().timestamp, SEC2MS(10))) {
	...
	if (m_udcounter == 5) {
		if (theApp.serverconnect->IsUDPSocketAvailable()
			&& (!m_lastudpstattime || HasTickCountElapsed(curTick, m_lastudpstattime, UDPSERVERSTATTIME)))
		{
		...
		m_udcounter = 0;
		if (theApp.serverconnect->IsUDPSocketAvailable())
			if (!m_lastudpsearchtime || HasTickCountElapsed(curTick, m_lastudpsearchtime, UDPSERVERREASKTIME))
				SendNextUDPPacket();
	}
	...
	// ZZ:DownloadManager -->
	if (!m_dwLastA4AFtime || HasTickCountElapsed(curTick, m_dwLastA4AFtime, MIN2MS(8))) {
...
{
	if (!m_lastcheckdiskspacetime || HasTickCountElapsed(::GetTickCount(), m_lastcheckdiskspacetime, DISKSPACERECHECKTIME))
		CheckDiskspace();
}
...
{
	const DWORD curTick = ::GetTickCount();
	if (!m_localServerReqQueue.IsEmpty() && IsTickCountDue(curTick, m_dwNextTCPSrcReq)) {
...
{
	return HasTickCountElapsed(::GetTickCount(), m_lastkademliafilerequest, KADEMLIAASKTIME);
}
