...
CUpDownClient* CUploadQueue::FindBestClientInQueue()
{
	...
	for (POSITION pos = waitinglist.GetHeadPosition(); pos != NULL;) {
		...
		if (HasTickCountElapsed(curTick, cur_client->GetLastUpRequest(), MAX_PURGEQUEUETIME) || !theApp.sharedfiles->GetFileByID(cur_client->GetUploadFileID())) {
...
void CUploadQueue::UpdateActiveClientsInfo(DWORD curTick)
{
	...
	while (!activeClients_hist.IsEmpty() && HasTickCountElapsed(curTick, activeClients_hist.Head().timestamp, SEC2MS(20))) {
	...
	if (activeClients_hist.Count() <= 1)
		...
		for (UINT_PTR ix = activeClients_hist.Count(); ix-- > 0;) {
			...
			if (HasTickCountElapsed(curTick, d.timestamp, SEC2MS(10)) && (tempMaxRemoved <= tempMax || tempMaxRemoved < m_MaxActiveClients))
				break;
			if (d.slots > tempMax)
				tempMax = d.slots;
			if (d.slots > tempMaxShortTime && !HasTickCountElapsed(curTick, d.timestamp, SEC2MS(10)))
...
void CUploadQueue::Process()
{
	...
	while (average_ur_hist.Count() > 3 && HasTickCountElapsed(curTick, average_ur_hist.Head().timestamp, SEC2MS(30))) {
...
bool CUploadQueue::ForceNewClient(bool allowEmptyWaitingQueue)
{
	...
	if (MaxSpeed == UNLIMITED) {
		// At startup the measured data rate can be only a few bytes per second.
		// Without a baseline this integer division becomes zero and prevents a
...
{
	const INT_PTR queueCount = waitinglist.GetCount();
	if (queueCount == 0) {
	...
	if (HasTickCountElapsed(curTick, m_dwLastCalculatedAverageCombinedFilePrioAndCredit, SEC2MS(5))) {
		...
		m_fAverageCombinedFilePrioAndCredit = sum / static_cast<float>(queueCount);
	}
...
bool CUploadQueue::CheckForTimeOver(const CUpDownClient *client)
{
	...
	if (thePrefs.TransferFullChunks()) {
		...
		if (client->GetScore(true, true) < GetMaxClientScore()) {
			const DWORD curTick = ::GetTickCount();
			if (IsTickCountDue(curTick, m_dwRemovedClientByScore)) {
				if (thePrefs.GetLogUlDlEvents())
...
void CUploadQueue::UpdateDatarates()
{
	...
	const DWORD curTick = ::GetTickCount();
	if (!HasTickCountElapsed(curTick, m_lastCalculatedDataRateTick, SEC2MS(1) / 2))
		return;
	m_lastCalculatedDataRateTick = curTick;
	if (average_ur_hist.Count() > 1) {
