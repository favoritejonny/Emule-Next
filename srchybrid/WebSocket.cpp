...
namespace
{
	static const DWORD MAX_HTTP_HEADER_SIZE = 16 * 1024;
	static const DWORD MAX_HTTP_CONTENT_SIZE = 2 * 1024 * 1024;
	static const DWORD MAX_HTTP_REQUEST_SIZE = MAX_HTTP_HEADER_SIZE + MAX_HTTP_CONTENT_SIZE;

	bool ParseHttpContentLength(const char *pValue, const char *pLineEnd, DWORD &rdwContentLength)
	{
		while (pValue < pLineEnd && (*pValue == ' ' || *pValue == '\t'))
			++pValue;
		if (pValue == pLineEnd || *pValue == '\r')
			return false;

		DWORD dwContentLength = 0;
		bool bHasDigits = false;
		for (; pValue < pLineEnd && *pValue != '\r'; ++pValue) {
			if (*pValue >= '0' && *pValue <= '9') {
				const DWORD dwDigit = static_cast<DWORD>(*pValue - '0');
				if (dwContentLength > (MAX_HTTP_CONTENT_SIZE - dwDigit) / 10)
					return false;
				dwContentLength = dwContentLength * 10 + dwDigit;
				bHasDigits = true;
			} else if (*pValue == ' ' || *pValue == '\t') {
				for (++pValue; pValue < pLineEnd && *pValue != '\r'; ++pValue)
					if (*pValue != ' ' && *pValue != '\t')
						return false;
				break;
			} else
		...
		if (!bHasDigits)
			return false;
		rdwContentLength = dwContentLength;
		return true;
	}
}
...
void CWebSocket::OnReceived(const void *pData, DWORD dwSize, const in_addr inad)
{
	...
	if ((pData == NULL && dwSize != 0) || dwSize > MAX_HTTP_REQUEST_SIZE || m_dwRecv > MAX_HTTP_REQUEST_SIZE - dwSize) {
		m_bValid = false;
		return;
	}

	const DWORD dwRequiredSize = m_dwRecv + dwSize;

	if (m_dwBufSize < dwRequiredSize) {
		// reallocate
		char *pNewBuf;
		const DWORD dwNewBufSize = dwRequiredSize <= MAX_HTTP_REQUEST_SIZE - SIZE_PRESERVE
			? dwRequiredSize + SIZE_PRESERVE : dwRequiredSize;
		try {
			pNewBuf = new char[dwNewBufSize];
		} catch (...) {
	...
	if (!m_dwHttpHeaderLen) {
		...
		for (DWORD dwPos = 0; dwPos < m_dwRecv; ++dwPos)
			if ('\n' == m_pBuf[dwPos]) {
				if (bPrevEndl) {
					...
					if (m_dwHttpHeaderLen > MAX_HTTP_HEADER_SIZE) {
						m_bValid = false;
						return;
					}
					...
					for (dwPos = 0; dwPos < m_dwHttpHeaderLen;) {
						const char *pPtr = static_cast<const char*>(memchr(&m_pBuf[dwPos], '\n', m_dwHttpHeaderLen - dwPos));
						if (!pPtr)
							break;
						const DWORD dwNextPos = static_cast<DWORD>(pPtr - m_pBuf);

						// check this header
						static const char szMatch[] = "content-length";
						const DWORD dwHeaderNameLength = sizeof szMatch - 1;
						if (dwNextPos - dwPos > dwHeaderNameLength
							&& !_strnicmp(&m_pBuf[dwPos], szMatch, dwHeaderNameLength)
							&& m_pBuf[dwPos + dwHeaderNameLength] == ':')
						{
							if (!ParseHttpContentLength(&m_pBuf[dwPos + dwHeaderNameLength + 1], pPtr, m_dwHttpContentLen)) {
								m_bValid = false;
								return;
							}
							break;
		...
		if (!m_dwHttpHeaderLen && m_dwRecv > MAX_HTTP_HEADER_SIZE) {
			m_bValid = false;
			return;
		}
	...
	const DWORD dwRequestSize = m_dwHttpHeaderLen + m_dwHttpContentLen;
	if (m_dwHttpHeaderLen && (!m_dwHttpContentLen || dwRequestSize <= m_dwRecv)) {
		OnRequestReceived(m_pBuf, m_dwHttpHeaderLen, m_pBuf + m_dwHttpHeaderLen, m_dwHttpContentLen, inad);

		if (m_bCanRecv && m_dwRecv > dwRequestSize) {
			// move our data
			m_dwRecv -= dwRequestSize;
			memmove(m_pBuf, m_pBuf + dwRequestSize, m_dwRecv);
		} else
			m_dwRecv = 0;
...
int StartSSL()
{
	...
	if (!ret) { // PSA_SUCCESS is 0
		const CStringA certPath(StrToUtf8(thePrefs.GetWebCertPath()));
		ret = mbedtls_x509_crt_parse_file(&srvcert, certPath);
		if (!ret) {
			const CStringA keyPath(StrToUtf8(thePrefs.GetWebKeyPath()));
	...
	if (ret)
		...
		if (psa_hash_compute(PSA_ALG_SHA_1, srvcert.raw.p, srvcert.raw.len, fingerprint,
			sizeof fingerprint, &fingerprintLength) == PSA_SUCCESS && fingerprintLength == sizeof fingerprint)
		{
			DebugLog(_T("Loaded certificate: %s"), (LPCTSTR)GetCertHash(fingerprint, (int)fingerprintLength));
		} else
			DebugLogWarning(_T("Loaded certificate, but could not calculate its SHA-1 fingerprint."));
	}
	return ret;
}
