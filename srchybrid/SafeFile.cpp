...
{
	if (m_nPosition > m_nFileSize || sizeof(uint16) > m_nFileSize - m_nPosition)
		AfxThrowFileException(CFileException::endOfFile, 0, GetFileName());
	uint16 nResult;
...
{
	if (m_nPosition > m_nFileSize || sizeof(uint32) > m_nFileSize - m_nPosition)
		AfxThrowFileException(CFileException::endOfFile, 0, GetFileName());
	uint32 nResult;
	memcpy(&nResult, &m_lpBuffer[m_nPosition], sizeof nResult);
...
{
	if (m_nPosition > m_nFileSize || sizeof(uint64) > m_nFileSize - m_nPosition)
		AfxThrowFileException(CFileException::endOfFile, 0, GetFileName());
	uint64 nResult;
	memcpy(&nResult, &m_lpBuffer[m_nPosition], sizeof nResult);
