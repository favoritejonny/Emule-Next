	...
	case 5: // progress
		{
			...
			if (!(HBITMAP)pCtrlItem->status || IsTickCountDue(curTick, pCtrlItem->dwUpdated)
...
void CDownloadListCtrl::OnContextMenu(CWnd*, CPoint point)
{
	...
	if (iSel >= 0) {
		...
		if (content != NULL && content->type == FILE_TYPE) {
			...
			m_FileMenu.EnableSubMenu(m_PrioMenu.m_hMenu, iFilesNotDone > 0 ? MF_ENABLED : MF_GRAYED);
			...
			if (thePrefs.IsExtControlsEnabled()) {
				...
				m_FileMenu.EnableSubMenu(m_PreviewMenu.m_hMenu, m_PreviewMenu.HasEnabledItems() ? MF_ENABLED : MF_GRAYED);
			...
			if (thePrefs.IsExtControlsEnabled()) {
				m_FileMenu.EnableSubMenu(m_SourcesMenu.m_hMenu, MF_ENABLED);
			...
			if (iPreviewMenuEntries)
				if (thePrefs.IsExtControlsEnabled() && !thePrefs.GetExtraPreviewWithMenu())
					VERIFY(m_PreviewMenu.RemoveSubMenu(PreviewWithMenu.m_hMenu));
				else
					VERIFY(m_FileMenu.RemoveSubMenu(PreviewWithMenu.m_hMenu));
...
// <-- ZZ:DownloadManager
		...
		int total;
		m_FileMenu.EnableSubMenu(m_PrioMenu.m_hMenu, MF_GRAYED);
		...
		if (thePrefs.IsExtControlsEnabled()) {
			m_FileMenu.EnableSubMenu(m_PreviewMenu.m_hMenu, MF_GRAYED);
			if (!thePrefs.GetPreviewPrio()) {
		...
		m_FileMenu.SetDefaultItem(UINT_MAX);
		if (m_SourcesMenu)
			m_FileMenu.EnableSubMenu(m_SourcesMenu.m_hMenu, MF_GRAYED);
	...
	for (ListItems::const_iterator it = m_ListItems.begin(); it != m_ListItems.end(); ++it) {
		...
		if (cur_item->type == FILE_TYPE) {
			...
			if (file->GetStatus() == PS_COMPLETE)
				fsize += static_cast<float>(file->GetFileSize());
		}
	}
	return fsize;
}
...
bool CDownloadListCtrl::ReportAvailableCommands(CList<int> &liAvailableCommands)
{
	...
	if (!HasTickCountElapsed(curTick, m_dwLastAvailableCommandsCheck, SEC2MS(3)) && !m_availableCommandsDirty)
		return false;
