...
void CPPgGeneral::DoDataExchange(CDataExchange *pDX)
{
	...
	DDX_Control(pDX, IDC_WEBSVEDIT, m_webServicesButton);
	DDX_Control(pDX, IDC_ED2KFIX, m_ed2kLinksButton);
}
...
void CPPgGeneral::LoadSettings()
{
	...
	GetDlgItem(IDC_STARTWIN)->EnableWindow(!thePrefs.IsPortableMode());
...
BOOL CPPgGeneral::OnApply()
{
	...
	thePrefs.SetAutoStart(IsDlgButtonChecked(IDC_STARTWIN) != 0);
...
{
	CWnd *pButton = GetDlgItem(IDC_ED2KFIX);
	if (pButton == NULL)
		return;

	const bool bPortable = thePrefs.IsPortableMode();
	const bool bNeedsAssociation = !bPortable && Ask4RegFix(true, false, true);
	CString strLabel(GetResString(IDS_ED2KLINKFIX));
	if (!bPortable && !bNeedsAssociation)
		strLabel.Insert(0, _T("\x2713 "));
	pButton->SetWindowText(strLabel);
	pButton->EnableWindow(!bPortable);
}
	...
	if (thePrefs.GetLanguageID() != newLangId) {
		if (!thePrefs.IsLanguageSupported(newLangId)) {
			const CString strInfo(
				_T("The selected language is not included in this package.\r\n\r\n")
				_T("eMule Next includes all supported translations locally. To request or contribute a translation, visit:\r\n")
				EMULE_NEXT_PROJECT_URL);
