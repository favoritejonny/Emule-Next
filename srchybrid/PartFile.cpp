...
void CPartFile::CreatePartFile(UINT cat)
{
	...
	if (thePrefs.GetSparsePartFiles(tempdirtouse)) {
...
void CPartFile::UpdateCompletedInfos(uint64 uTotalGaps)
{
	...
	if (m_gaplist.IsEmpty()) {
		...
		m_percentcompleted = static_cast<float>(floor((1.0 - static_cast<double>(uTotalGaps) / static_cast<double>(m_nFileSize)) * 1000.0) / 10.0);
...
void CPartFile::DrawStatusBar(CDC &dc, const CRect &rect, bool bFlat) /*const*/
{
	...
	if (status == PS_COMPLETE || status == PS_COMPLETING) {
		...
		// green progress
		float blockpixel = static_cast<float>(rect.Width()) / static_cast<float>(m_nFileSize);
		float width = static_cast<float>(static_cast<uint64>(m_nFileSize) - allgaps) * blockpixel + 0.5f;
		if (!bFlat) {
	...
	if (GetFileOp() != PFOP_NONE) {
		...
		LONG width = (LONG)(static_cast<float>(GetFileOpProgress()) * static_cast<float>(rcFileOpProgress.Width()) / 100.0F + 0.5F);
...
uint32 CPartFile::Process(uint32 reducedownload, UINT icounter/*in percent*/)
{
	...
	// Unsigned elapsed-time arithmetic keeps the buffer flush schedule correct
	// when the Windows tick counter wraps.
	if (m_nTotalBufferData > thePrefs.GetFileBufferSize() || HasTickCountElapsed(curTick, m_nLastBufferFlushTime, thePrefs.GetFileBufferTimeLimit()))
		FlushBuffer();
	//If data keeps arriving, flush to disk sometimes for extra safety
	if (m_nFileFlushTime && HasTickCountElapsed(curTick, m_nFileFlushTime, SEC2MS(31)) && m_hWrite != INVALID_HANDLE_VALUE) {
	...
	if (icounter < 10) {
		...
		for (POSITION pos = srclist.GetHeadPosition(); pos != NULL;) {
			...
			case DS_LOWTOLOWIP: // Check if something has changed with our or their ID state.
				...
				if (cur_src->HasLowID() && !theApp.CanDoCallback(cur_src)) {
					...
					if (HasTickCountElapsed(curTick, m_lastpurgetime, SEC2MS(30)) && (GetSourceCount() >= GetMaxSources() * 4 / 5)) {
			...
			case DS_NONEEDEDPARTS:
				// To Mods, please stop instantly removing these sources.
				// This causes sources to pop in and out creating extra overhead!
				if (HasTickCountElapsed(curTick, m_lastpurgetime, SEC2MS(40))) {
					m_lastpurgetime = curTick;
			...
			case DS_ONQUEUE:
				...
				if (cur_src->IsRemoteQueueFull()) {
					if (HasTickCountElapsed(curTick, m_lastpurgetime, MIN2MS(1)) && (GetSourceCount() >= (GetMaxSources() * 4 / 5))) {
						theApp.downloadqueue->RemoveSource(cur_src);
						m_lastpurgetime = curTick;
						break;
					}
				...
				if (theApp.IsConnected() && cur_src->GetTimeUntilReask() < MIN2MS(2) && cur_src->GetTimeUntilReask() > SEC2MS(1) && HasTickCountElapsed(curTick, cur_src->GetLastTriedToConnectTime(), MIN2MS(20))) // ZZ:DownloadManager (one re-ask timestamp for each file)
			...
			case DS_WAITCALLBACKKAD:
				if (theApp.IsConnected() && cur_src->GetTimeUntilReask() == 0) { // ZZ:DownloadManager (one re-ask timestamp for each file)
					...
					} else if (HasTickCountElapsed(curTick, cur_src->GetLastTriedToConnectTime(), MIN2MS(20))) {
		...
		if (!m_bLocalSrcReqQueued && (!m_LastSearchTime || HasTickCountElapsed(curTick, m_LastSearchTime, SERVERREASKTIME)) && theApp.serverconnect->IsConnected()
...
void CPartFile::OpenFile() const
{
	if (m_pCollection) {
		...
		dialog.DoModal();
	} else if (CheckFileOpen(GetFullName(), GetFileName()))
...
void CPartFile::FlushBuffer(bool bForceICH, bool bNoAICH)
{
	...
	try {
		...
		if (m_bUpdateMet) {
			if (IsTickCountDue(m_nLastBufferFlushTime, m_nNextMetFlushTime)) {
...
void CPartFile::UpdateDisplayedInfo(bool force)
{
	if (!theApp.IsClosing()) {
		...
		if (force || HasTickCountElapsed(curTick, m_lastRefreshedDLDisplay, MINWAIT_BEFORE_DLDISPLAY_WINDOWUPDATE + m_random_update_wait)) {
