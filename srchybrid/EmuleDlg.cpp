...
namespace
{
	int ExcludeVisibleChildAreas(CWnd* parent, CDC* dc)
	{
		if (parent == NULL || dc == NULL)
			return 0;
...
CemuleDlg::~CemuleDlg()
{
	...
	if (m_hSmallIcon)
		VERIFY(::DestroyIcon(m_hSmallIcon));
	DestroyIconsArr(m_connicons, _countof(m_connicons));
	DestroyIconsArr(m_ed2kicons, _countof(m_ed2kicons));
	DestroyIconsArr(m_kadicons, _countof(m_kadicons));
...
BOOL CemuleDlg::OnInitDialog()
{
	...
	// set title
	SetWindowText(theApp.GetProductDisplayName());
	...
	// A crash report remains entirely local until the user explicitly chooses
	// to open the eMule Next project page on the following start.
	theCrashDumper.ShowPendingReport();
...
{
	// The previous DNS service belongs to the historical eMule project. Until
	// eMule Next ships its own signed update feed, update checks stay local and
	// the explicit command opens the project's official page.
	if (manual)
...
UINT CemuleDlg::GetEd2kConnectionStateIconIndex() const
{
	if (!theApp.serverconnect->IsConnected())
		return 0;
	return theApp.serverconnect->IsLowID() ? 1 : 2;
}

UINT CemuleDlg::GetKadConnectionStateIconIndex() const
{
	if (!Kademlia::CKademlia::IsConnected())
		return Kademlia::CKademlia::IsRunning() ? 1 : 0;
	return Kademlia::CKademlia::IsFirewalled() ? 2 : 3;
}

void CemuleDlg::ShowConnectionStateIcon()
{
	UINT uEd2kIconIdx = GetEd2kConnectionStateIconIndex();
	UINT uKadIconIdx = GetKadConnectionStateIconIndex();
	ASSERT(uEd2kIconIdx < _countof(m_ed2kicons));
	ASSERT(uKadIconIdx < _countof(m_kadicons));
	statusbar->SetIcon(SBarConnected, m_ed2kicons[uEd2kIconIdx]);
	statusbar->SetIcon(SBarKad, m_kadicons[uKadIconIdx]);
}

CString CemuleDlg::GetEd2kConnectionStateString() const
{
	UINT ed2k = IDS_DISCONNECTED;
	if (theApp.serverconnect->IsConnected())
		ed2k = IDS_CONNECTED;
	...
	CString state;
	state.Format(_T("eD2K:%s"), (LPCTSTR)GetResString(ed2k));
	return state;
}

CString CemuleDlg::GetKadConnectionStateString() const
{
	UINT kad = IDS_DISCONNECTED;

	if (Kademlia::CKademlia::IsConnected())
		kad = Kademlia::CKademlia::IsFirewalled() ? IDS_FIREWALLED : IDS_CONNECTED;
	else if (Kademlia::CKademlia::IsRunning())
		kad = IDS_CONNECTING;

	CString state;
	state.Format(_T("Kad:%s"), (LPCTSTR)GetResString(kad));
	return state;
}

CString CemuleDlg::GetConnectionStateString()
{
	return GetEd2kConnectionStateString() + _T('|') + GetKadConnectionStateString();
}
...
void CemuleDlg::ShowConnectionState()
{
	...
	statusbar->SetText(GetEd2kConnectionStateString(), SBarConnected, 0);
	statusbar->SetText(GetKadConnectionStateString(), SBarKad, 0);
...
CString CemuleDlg::GetUpDatarateString(UINT uUpDatarate)
{
	...
	if (thePrefs.ShowOverhead())
		szBuff.Format(_T("%.1f (%.1f)"), static_cast<double>(m_uUpDatarate) / 1024.0, static_cast<double>(theStats.GetUpDatarateOverhead()) / 1024.0);
	else
		szBuff.Format(_T("%.1f"), static_cast<double>(m_uUpDatarate) / 1024.0);
	return szBuff;
}
...
CString CemuleDlg::GetDownDatarateString(UINT uDownDatarate)
{
	...
	CString szBuff;
	if (thePrefs.ShowOverhead())
		szBuff.Format(_T("%.1f (%.1f)"), static_cast<double>(m_uDownDatarate) / 1024.0, static_cast<double>(theStats.GetDownDatarateOverhead()) / 1024.0);
	else
		szBuff.Format(_T("%.1f"), static_cast<double>(m_uDownDatarate) / 1024.0);
	return szBuff;
}
...
CString CemuleDlg::GetTransferRateString()
{
	...
	if (thePrefs.ShowOverhead())
		szBuff.Format(GetResString(IDS_UPDOWN)
			, static_cast<double>(m_uUpDatarate) / 1024.0, static_cast<double>(theStats.GetUpDatarateOverhead()) / 1024.0
...
void CemuleDlg::ShowTransferRate(bool bForceAll)
{
	...
	if (TrayIconVisible() || bForceAll) {
		// set tray icon
		int iDownRatePercent = (int)ceil((static_cast<double>(m_uDownDatarate) / 10.24) / static_cast<double>(thePrefs.GetMaxGraphDownloadRate()));
		UpdateTrayIcon(min(iDownRatePercent, 100));

		CString buffer;
		buffer.Format(_T("%s (%s)\r\n%s")
			, theApp.GetProductDisplayName()
	...
	if (IsWindowVisible() && thePrefs.ShowRatesOnTitle()) {
		CString szBuff;
		szBuff.Format(_T("(U:%.1f D:%.1f) %s"), static_cast<float>(m_uUpDatarate) / 1024.0f, static_cast<float>(m_uDownDatarate) / 1024.0f, theApp.GetProductDisplayName());
	...
	if (IsWindowVisible() && thePrefs.IsDynUpEnabled()) {
		...
		if (strState.IsEmpty()) {
			if (lastPing.lowest > 0 && !thePrefs.IsDynUpUseMillisecondPingTolerance())
				strState.Format(_T("%.1f | %ums | %u%%"), static_cast<float>(lastPing.currentLimit) / 1024.0f, lastPing.latency, lastPing.latency * 100 / lastPing.lowest);
			else
				strState.Format(_T("%.1f | %ums"), static_cast<float>(lastPing.currentLimit) / 1024.0f, lastPing.latency);
		}
...
void CemuleDlg::SetStatusBarPartsSize()
{
	...
	int aiWidths[7] =
	{
		rect.right - 760 - ussShift,
		rect.right - 515 - ussShift,
		rect.right - 265 - ussShift,
		rect.right - 145 - ussShift,
	...
	case OP_CLCOMMAND:
		{
			...
			if (clcommand == _T("connect"))
				...
				if (file) {
					...
					_ftprintf(file, (LPCTSTR)GetResString(IDS_UPDOWNSMALL), static_cast<float>(theApp.uploadqueue->GetDatarate()) / 1024.0f, static_cast<float>(theApp.downloadqueue->GetDatarate()) / 1024.0f);
...
{
	const int savedDc = ExcludeVisibleChildAreas(this, pDC);
	BOOL erased = FALSE;
	if (CNextTheme::GetTheme() != entModernAurora) {
		erased = __super::OnEraseBkgnd(pDC);
	} else {
		CRect rcClient;
		GetClientRect(&rcClient);
		TRIVERTEX vertices[2] = {
			{ rcClient.left, rcClient.top, 0xB500, 0xE700, 0xFF00, 0x0000 },
			{ rcClient.right, rcClient.bottom, 0xE400, 0xC100, 0xFF00, 0x0000 }
		};
		GRADIENT_RECT gradient = { 0, 1 };
		erased = ::GradientFill(pDC->m_hDC, vertices, _countof(vertices), &gradient, 1,
...
{
	// Keep Windows high-contrast settings authoritative. The eMule Next palette
	// is intentionally applied only when it can improve the normal interface.
	...
	static CBrush s_brNextLightCanvas(RGB(234, 244, 248));
	static CBrush s_brNextLightSurface(RGB(255, 255, 255));
	static CBrush s_brAuroraCanvas(RGB(224, 231, 255));
	static CBrush s_brAuroraSurface(RGB(248, 244, 255));
	const bool bAuroraTheme = CNextTheme::GetTheme() == entModernAurora;
	const COLORREF crText = (pWnd != NULL && !pWnd->IsWindowEnabled())
		? (bAuroraTheme ? RGB(135, 139, 176) : ::GetSysColor(COLOR_GRAYTEXT))
		: (bAuroraTheme ? RGB(65, 58, 132) : RGB(29, 58, 73));
	const COLORREF crCanvas = bAuroraTheme ? RGB(224, 231, 255) : RGB(234, 244, 248);
	const COLORREF crSurface = bAuroraTheme ? RGB(248, 244, 255) : RGB(255, 255, 255);
	HBRUSH hbrCanvas = (HBRUSH)(bAuroraTheme ? s_brAuroraCanvas.GetSafeHandle() : s_brNextLightCanvas.GetSafeHandle());
	HBRUSH hbrSurface = (HBRUSH)(bAuroraTheme ? s_brAuroraSurface.GetSafeHandle() : s_brNextLightSurface.GetSafeHandle());

	switch (nCtlColor) {
	case CTLCOLOR_DLG:
	case CTLCOLOR_STATIC:
		pDC->SetTextColor(crText);
		pDC->SetBkColor(crCanvas);
		return hbrCanvas;
	case CTLCOLOR_EDIT:
	case CTLCOLOR_LISTBOX:
		pDC->SetTextColor(crText);
		pDC->SetBkColor(crSurface);
		return hbrSurface;
	}
...
{
	// Use separate scalable icons for the taskbar/Alt+Tab and title bar. This
	// avoids Windows shrinking the large application icon into a blurry 16px
	// image and replaces the remaining legacy mule artwork in the window frame.
	if (m_hIcon)
		VERIFY(::DestroyIcon(m_hIcon));
	if (m_hSmallIcon)
		VERIFY(::DestroyIcon(m_hSmallIcon));
	m_hIcon = theApp.LoadIcon(_T("NEXT_MULE_HEAD"),
		::GetSystemMetrics(SM_CXICON), ::GetSystemMetrics(SM_CYICON));
	m_hSmallIcon = theApp.LoadIcon(_T("NEXT_MULE_HEAD"),
	...
	// The status bar presents eD2K and Kad separately, so their state remains
	// recognizable at a glance instead of relying on a combined status icon.
	DestroyIconsArr(m_ed2kicons, _countof(m_ed2kicons));
	m_ed2kicons[0] = theApp.LoadIcon(_T("Ed2kStatusDisconnected"), 16, 16);
	m_ed2kicons[1] = theApp.LoadIcon(_T("Ed2kStatusLowId"), 16, 16);
	m_ed2kicons[2] = theApp.LoadIcon(_T("Ed2kStatusHighId"), 16, 16);
	DestroyIconsArr(m_kadicons, _countof(m_kadicons));
	m_kadicons[0] = theApp.LoadIcon(_T("KadStatusDisconnected"), 16, 16);
	m_kadicons[1] = theApp.LoadIcon(_T("KadStatusConnecting"), 16, 16);
	m_kadicons[2] = theApp.LoadIcon(_T("KadStatusFirewalled"), 16, 16);
	m_kadicons[3] = theApp.LoadIcon(_T("KadStatusConnected"), 16, 16);
	ShowConnectionStateIcon();
	...
	if (m_icoSysTrayLowID)
		VERIFY(::DestroyIcon(m_icoSysTrayLowID));
	// The icon beside the Windows clock is an eMule Next mule head. Its colour
	// still communicates connection quality, and the existing transfer meter
	// remains drawn over it by UpdateTrayIcon.
	m_icoSysTrayConnected = theApp.LoadIcon(_T("NEXT_MULE_TRAY_CONNECTED"), 16, 16);
	m_icoSysTrayDisconnected = theApp.LoadIcon(_T("NEXT_MULE_TRAY_DISCONNECTED"), 16, 16);
	m_icoSysTrayLowID = theApp.LoadIcon(_T("NEXT_MULE_TRAY_LOWID"), 16, 16);
	ShowTransferRate(true);
...
void CemuleDlg::ShowToolPopup(bool toolsonly)
{
	...
	const CString strMenuTitle = toolsonly
		? GetResString(IDS_TOOLS)
		: CString(CemuleApp::GetProductDisplayName());
	menu.AddMenuTitle(strMenuTitle, true);

	CTitledMenu Links;
	Links.CreateMenu();
	Links.AddMenuTitle(NULL, true);
	Links.AppendMenu(MF_STRING, MP_HM_LINK1, GetResString(IDS_HM_LINKHP), _T("NEXT_WEB"));
	Links.AppendMenu(MF_STRING, MP_HM_LINK2, GetResString(IDS_HM_LINKFAQ), _T("NEXT_WEB"));
	Links.AppendMenu(MF_STRING, MP_HM_LINK3, GetResString(IDS_HM_LINKVC), _T("NEXT_WEB"));
	...
	if (!toolsonly) {
		...
		menu.AppendMenu(MF_STRING, MP_HM_KAD, GetResString(IDS_EM_KADEMLIA), _T("NEXT_MULE_HEAD"));
	...
	menu.AppendMenu(MF_STRING, MP_HM_OPENINC, GetResString(IDS_OPENINC) + _T("..."), _T("NEXT_INCOMING"));
	menu.AppendMenu(MF_STRING, MP_HM_CONVERTPF, GetResString(IDS_IMPORTSPLPF) + _T("..."), _T("NEXT_CONVERT"));
	menu.AppendMenu(MF_STRING, MP_HM_1STSWIZARD, GetResString(IDS_WIZ1) + _T("..."), _T("NEXT_WIZARD"));
	menu.AppendMenu(MF_STRING, MP_HM_IPFILTER, GetResString(IDS_IPFILTER) + _T("..."), _T("NEXT_IPFILTER"));
...
BOOL CemuleApp::IsIdleMessage(MSG *pMsg)
{
	...
	if (pMsg->message == WM_TIMER) {
		...
		if (HasTickCountElapsed(curTick, s_dwLastIdleMessage, SEC2MS(5))) {
...
LRESULT CemuleDlg::OnKickIdle(WPARAM, LPARAM lIdleCount)
{
	...
	if (m_pSplashWnd) {
		if (HasTickCountElapsed(::GetTickCount(), m_dwSplashTime, static_cast<DWORD>(SEC2MS(2.5)))) {
...
void CemuleDlg::SetTaskbarIconColor()
{
	...
	uint16 iBrightness = static_cast<uint16>(sqrt(((static_cast<float>(iRed) * iRed * 0.241f) + (static_cast<float>(iGreen) * iGreen * 0.691f) + (static_cast<float>(iBlue) * iBlue * 0.068f))));
