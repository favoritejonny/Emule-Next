...
void CStatistics::RecordRate()
{
	...
	// Limit the history while preserving the newest sample. Comparing elapsed
	// time rather than absolute ticks keeps this valid when the tick counter wraps.
	while (downrate_hist.Count() > 1 && HasTickCountElapsed(curTick, downrate_hist.Head().timestamp, avg))
		downrate_hist.RemoveHead();
	while (uprate_hist.Count() > 1 && HasTickCountElapsed(curTick, uprate_hist.Head().timestamp, avg))
...
float CStatistics::GetAvgDownloadRate(int averageType)
{
	...
	case AVG_TOTAL:
		if (theStats.transferStarttime > 0) {
			...
			if (running >= 5)
				return (static_cast<float>(theStats.sessionReceivedBytes) / (1.024f * static_cast<float>(running)) + thePrefs.GetConnAvgDownRate()) / 2.0F;
		}
	...
	case AVG_SESSION:
		if (theStats.transferStarttime > 0) {
			DWORD running = ::GetTickCount() - theStats.transferStarttime;
			if (running >= 5)
				return static_cast<float>(theStats.sessionReceivedBytes) / (1.024f * static_cast<float>(running));
		}
		break;
	case AVG_TIME:
		if (!downrate_hist.IsEmpty()) {
			...
			if (running > 0)
				return static_cast<float>(downrate_hist.Tail().datalen - downrate_hist.Head().datalen) / (1.024f * static_cast<float>(running));
		}
	}
	return 0.0F;
}
...
float CStatistics::GetAvgUploadRate(int averageType)
{
	...
	case AVG_TOTAL:
		if (theStats.transferStarttime > 0) {
			DWORD running = ::GetTickCount() - theStats.transferStarttime;
			if (running >= 5)
				return (static_cast<float>(theStats.sessionSentBytes) / (1.024f * static_cast<float>(running)) + thePrefs.GetConnAvgUpRate()) / 2.0F;
		}
	...
	case AVG_SESSION:
		if (theStats.transferStarttime) {
			DWORD running = ::GetTickCount() - theStats.transferStarttime;
			if (running >= 5)
				return static_cast<float>(theStats.sessionSentBytes) / (1.024f * static_cast<float>(running));
		}
		break;
	case AVG_TIME:
		if (!uprate_hist.IsEmpty()) {
			...
			if (running > 0)
				return static_cast<float>(uprate_hist.Tail().datalen - uprate_hist.Head().datalen) / (1.024f * static_cast<float>(running));
		}
	}
	return 0.0F;
}
...
void CStatistics::CompDownDatarateOverhead()
{
	...
	while (HasTickCountElapsed(curTick, m_AverageDDRO_hist.Head().timestamp, MAXAVERAGETIME)) {
	...
	if (m_AverageDDRO_hist.Count() > 10) {
		...
		if (HasTickCountElapsed(curTick, head.timestamp, 1)) {
...
void CStatistics::CompUpDatarateOverhead()
{
	...
	while (HasTickCountElapsed(curTick, m_AverageUDRO_hist.Head().timestamp, MAXAVERAGETIME)) {
