...
bool CUploadDiskIOThread::AssociateFile(CKnownFile *pFile)
{
	...
	if (pFile && pFile->m_hRead == INVALID_HANDLE_VALUE && !pFile->bNoNewReads) {
		CString fullname = (pFile->IsPartFile())
			...
			: pFile->GetFilePath();
		const CString fileSystemPath = MakeLongPathCompatible(fullname);
		pFile->m_hRead = ::CreateFile(fileSystemPath, GENERIC_READ, FILE_SHARE_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
