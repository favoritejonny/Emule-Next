...
void CBarShader::BuildModifiers()
{
	...
	const int depth = 7 - static_cast<int>(min(m_used3dlevel, 5U));
	const int count = max(1, HALF(m_iHeight));
	const double piOverDepth = M_PI / depth;
	double base = piOverDepth * (depth / 2.0 - 1);
	double increment = count > 1 ? piOverDepth / (count - 1) : 0.0;

	m_Modifiers = new float[count];
	for (int i = 0; i < count; ++i)
		m_Modifiers[i] = static_cast<float>(sin(base + i * increment));
}
...
void CBarShader::SetWidth(int width)
{
	if (m_iWidth != width) {
		m_iWidth = width;
		m_dPixelsPerByte = (static_cast<uint64>(m_uFileSize) > 0) ? static_cast<double>(m_iWidth) / static_cast<double>(m_uFileSize) : 0.0;
		m_dBytesPerPixel = (m_iWidth > 0) ? static_cast<double>(m_uFileSize) / static_cast<double>(m_iWidth) : 0.0;
	}
}
