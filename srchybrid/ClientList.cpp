...
{
	DWORD dwBantime;
	return m_bannedList.Lookup(dwIP, dwBantime) && !HasTickCountElapsed(::GetTickCount(), dwBantime, CLIENTBANTIME);
}
...
void CClientList::Process()
{
	...
	if (HasTickCountElapsed(curTick, m_dwLastBanCleanUp, BAN_CLEANUP_TIME)) {
		...
		for (POSITION pos = m_bannedList.GetStartPosition(); pos != NULL;) {
			...
			if (HasTickCountElapsed(curTick, dwBantime, CLIENTBANTIME))
	...
	//
	if (HasTickCountElapsed(curTick, m_dwLastTrackedCleanUp, TRACKED_CLEANUP_TIME)) {
		...
		for (POSITION pos = m_trackedClientsMap.GetStartPosition(); pos != NULL;) {
			...
			CDeletedClient *pResult;
			m_trackedClientsMap.GetNextAssoc(pos, nKey, pResult);
			if (HasTickCountElapsed(curTick, pResult->m_dwInserted, KEEPTRACK_TIME)) {
				m_trackedClientsMap.RemoveKey(nKey);
				delete pResult;
			}
		}
...
void CClientList::CleanUpClientList()
{
	...
	const DWORD curTick = ::GetTickCount();
	if (HasTickCountElapsed(curTick, m_dwLastClientCleanUp, CLIENTLIST_CLEANUP_TIME)) {
...
void CClientList::AddKadFirewallRequest(uint32 dwIP)
{
	...
	while (!listFirewallCheckRequests.IsEmpty() && HasTickCountElapsed(curTick, listFirewallCheckRequests.GetTail().dwInserted, SEC2MS(180)))
...
bool CClientList::IsKadFirewallCheckIP(uint32 dwIP) const
{
	...
	for (POSITION pos = listFirewallCheckRequests.GetHeadPosition(); pos != NULL;) {
		...
		if (HasTickCountElapsed(curTick, iptick.dwInserted, SEC2MS(180)))
			break;
		if (iptick.dwIP == dwIP)
			return true;
	}
...
void CClientList::ProcessConnectingClientsList()
{
	...
	for (POSITION pos = m_liConnectingClients.GetHeadPosition(); pos != NULL;) {
		...
		if (HasTickCountElapsed(curTick, cc.dwInserted, SEC2MS(45))) {
...
void CClientList::AddTrackCallbackRequests(uint32 dwIP)
{
	...
	while (!listDirectCallbackRequests.IsEmpty() && HasTickCountElapsed(curTick, listDirectCallbackRequests.GetTail().dwInserted, SEC2MS(180)))
...
bool CClientList::AllowCalbackRequest(uint32 dwIP) const
{
	...
	for (POSITION pos = listDirectCallbackRequests.GetHeadPosition(); pos != NULL;) {
		...
		if (iptick.dwIP == dwIP && !HasTickCountElapsed(curTick, iptick.dwInserted, SEC2MS(180)))
			return false;
	}
	return true;
}
