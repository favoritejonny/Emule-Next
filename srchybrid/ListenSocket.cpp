...
bool CClientReqSocket::CheckTimeOut()
{
	...
	if (m_nOnConnect == SS_Half) {
		...
		if (!HasTickCountElapsed(curTick, timeout_timer, CEMSocket::GetTimeOut() * 4))
			return false;
	...
	if (client)
		if (client->GetKadState() == KS_CONNECTED_BUDDY)
			uTimeout += MIN2MS(15);
		else if (client->IsDownloading() && !HasTickCountElapsed(curTick, client->GetUpStartTime(), 4 * CONNECTION_TIMEOUT))
	...
	if (!HasTickCountElapsed(curTick, timeout_timer, uTimeout))
		return false;
	timeout_timer = curTick;
	CString str;
...
// we currently also rely on this for multithreading; rework synchronization if this ever changes
	if (HasTickCountElapsed(::GetTickCount(), deltimer, SEC2MS(10)))
		delete this;
}
