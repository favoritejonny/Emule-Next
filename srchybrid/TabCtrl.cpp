...
namespace
{
const COLORREF kCommunityAccentColor = RGB(47, 207, 232);

bool IsHighContrastMode()
{
	HIGHCONTRAST highContrast = {};
	highContrast.cbSize = sizeof highContrast;
	return ::SystemParametersInfo(SPI_GETHIGHCONTRAST, sizeof highContrast, &highContrast, 0) != FALSE
...
void TabControl::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	// Keep category-specific text colours intact while adding a subtle, visible
	// active-tab cue. High contrast mode remains exclusively system controlled.
	if (bSelected && !IsHighContrastMode()) {
		CRect rcAccent(rcFullItem);
		if (rcAccent.Width() > 16)
			rcAccent.DeflateRect(8, 0);
		rcAccent.top = rcAccent.bottom - 3;
		pDC->FillSolidRect(&rcAccent, kCommunityAccentColor);
	}
