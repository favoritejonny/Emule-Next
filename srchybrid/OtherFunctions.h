...
// Builds a path without the fixed-size buffers used by the legacy CRT helper.
CString		MakePath(LPCTSTR drive, LPCTSTR dir, LPCTSTR fname, LPCTSTR ext);
// Returns an extended-length path when an absolute local or UNC path exceeds MAX_PATH.
// The returned string is intended for Win32 file-system APIs, not for display or storage.
CString		MakeLongPathCompatible(LPCTSTR path);
