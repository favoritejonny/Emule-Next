...
//		m_eventPaused.Lock();
		...
		if (thePrefs.GetMaxUpload() == UNLIMITED) {
			...
			if (nCanSend > 0) {
				...
				if (nBusy > 2 && iBusyFraction > 24 && nSlotsBusyLevel < 255) {
					...
					if (thePrefs.GetVerbose() && nSlotsBusyLevel % 25 == 0 && lotsOfLog)
						theApp.QueueDebugLogLine(false, _T("Throttler: nSlotsBusyLevel: %i Guessed limit: %0.5f changesCount: %i loopsCount: %i"), nSlotsBusyLevel, static_cast<double>(nEstiminatedDataRate) / 1024.0, changesCount, loopsCount);
			...
			if (nUploadStartTime == 0) {
				...
				// timeGetTime wraps roughly every 49 days. Compare elapsed time by
				// subtraction so automatic upload calibration cannot begin early at
				// the wrap boundary.
			} else if (theApp.uploadqueue && HasTickCountElapsed(timeGetTime(), nUploadStartTime, SEC2MS(60))) {
				if (nEstiminatedDataRate == 0) { // no auto limit was set yet
					if (nSlotsBusyLevel >= 250) { // sockets indicated that the BW limit has been reached
						...
						if (thePrefs.GetVerbose() && estimateChangedLog)
							theApp.QueueDebugLogLine(false, _T("Throttler: Set initial estimated limit to %0.5f changesCount: %i loopsCount: %i"), static_cast<double>(nEstiminatedDataRate) / 1024.0, changesCount, loopsCount);
						changesCount = 0;
						loopsCount = 0;
					}
					...
					if (thePrefs.GetVerbose() && estimateChangedLog)
						theApp.QueueDebugLogLine(false, _T("Throttler: REDUCED limit #%i by %i bytes to: %0.5f changesCount: %i loopsCount: %i"), numberOfConsecutiveDownChanges, changeDelta, static_cast<double>(nEstiminatedDataRate) / 1024.0, changesCount, loopsCount);
					...
					if (thePrefs.GetVerbose() && estimateChangedLog)
						theApp.QueueDebugLogLine(false, _T("Throttler: INCREASED limit #%i by %i bytes to: %0.5f changesCount: %i loopsCount: %i"), numberOfConsecutiveUpChanges, changeDelta, static_cast<double>(nEstiminatedDataRate) / 1024.0, changesCount, loopsCount);
			...
			if (nCanSend == nBusy && GetStandardListSize() > 0 && nSlotsBusyLevel < 125) {
				...
				if (thePrefs.GetVerbose() && lotsOfLog)
					theApp.QueueDebugLogLine(false, _T("Throttler: nSlotsBusyLevel: %i Guessed limit: %0.5f changesCount %i loopsCount: %i (set due to all slots busy)"), nSlotsBusyLevel, static_cast<double>(nEstiminatedDataRate) / 1024.0, changesCount, loopsCount);
			}
		}
...
#define TIME_BETWEEN_UPLOAD_LOOPS 1
		...
		if (allowedDataRate == _UI32_MAX || spendingRate >= SEC2MS(1) || (allowedDataRate | nEstiminatedDataRate) == 0)
			...
			if (allowedDataRate)
				...
				sleepTime = (DWORD)ceil(static_cast<double>(SEC2MS(1) - spendingRate) / static_cast<double>(allowedDataRate));
			else
