		...
		for (INT_PTR i = 0; i < (INT_PTR)iVisibleNodes; ++i) {
			...
			if (sEntry->m_dwAskedContactsTime > 0) {
				if (sEntry->m_uRespondedContact > 0)
					...
					iIconIdx = HasTickCountElapsed(::GetTickCount(), sEntry->m_dwAskedContactsTime, SEC2MS(3)) ? 3 : 2; // red or yellow
				...
				case Kademlia::CSearch::NOTES:
					{
						...
						if (sEntry->m_uRespondedSearchItem > 0)
							...
							nOverlayImage = HasTickCountElapsed(::GetTickCount(), sEntry->m_dwAskedSearchItemTime, SEC2MS(5)) ? 2 : 0;
...
void CKadLookupGraph::UpdateToolTip()
{
	...
	if (m_iHotItemIdx >= 0) {
		...
		if (sEntry->m_dwAskedContactsTime > 0) {
			if (sEntry->m_uRespondedContact == 0) {
				if (HasTickCountElapsed(::GetTickCount(), sEntry->m_dwAskedContactsTime, SEC2MS(3)))
		...
		if (sEntry->m_dwAskedSearchItemTime > 0) {
			if (sEntry->m_uRespondedSearchItem == 0) {
				if (HasTickCountElapsed(::GetTickCount(), sEntry->m_dwAskedSearchItemTime, SEC2MS(5)))
