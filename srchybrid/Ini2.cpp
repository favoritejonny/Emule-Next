...
CString CIni::GetStringUTF8(LPCTSTR lpszEntry, LPCTSTR lpszDefault, LPCTSTR lpszSection)
{
	...
	CStringA strUTF8;
	CStringA strSection(m_strSection);
	CStringA strEntry(lpszEntry);
	CStringA strDefault(lpszDefault ? lpszDefault : _T(""));
	CStringA strFileName(m_strFileName);
	DWORD dwBufferSize = MAX_INI_BUFFER;
	for (;;) {
		DWORD dwChars = ::GetPrivateProfileStringA(strSection, strEntry, strDefault,
			strUTF8.GetBuffer(dwBufferSize), dwBufferSize, strFileName);
		strUTF8.ReleaseBuffer(static_cast<int>(dwChars));
		// A single profile value is truncated only when the return value is nSize - 1.
		if (dwChars < dwBufferSize - 1 || dwBufferSize >= _UI16_MAX)
			break;
		dwBufferSize += MAX_INI_BUFFER;
	}
...
DWORD CIni::GetDWORD(LPCTSTR lpszEntry, DWORD nDefault, LPCTSTR lpszSection)
{
	TCHAR szDefault[MAX_PATH];
	_sntprintf(szDefault, _countof(szDefault), _T("%lu"), nDefault);
	szDefault[_countof(szDefault) - 1] = _T('\0');

	const CString strValue(GetString(lpszEntry, szDefault, lpszSection));
	LPCTSTR pszValue = strValue;
	while (_istspace(*pszValue))
		++pszValue;
	if (*pszValue == _T('\0'))
		return nDefault;
	...
	TCHAR *pszEnd = NULL;
	if (*pszValue == _T('-')) {
		const LONGLONG nValue = _tcstoi64(pszValue, &pszEnd, 10);
		while (pszEnd != NULL && _istspace(*pszEnd))
			++pszEnd;
		if (pszEnd != pszValue && *pszEnd == _T('\0') && nValue >= INT32_MIN && nValue <= INT32_MAX)
			return static_cast<DWORD>(static_cast<LONG>(nValue));
	} else {
		const ULONGLONG nValue = _tcstoui64(pszValue, &pszEnd, 10);
		while (pszEnd != NULL && _istspace(*pszEnd))
			++pszEnd;
		if (pszEnd != pszValue && *pszEnd == _T('\0') && nValue <= _UI32_MAX)
...
{
	return static_cast<WORD>(GetDWORD(lpszEntry, nDefault, lpszSection));
}
...
void CIni::WriteDWORD(LPCTSTR lpszEntry, DWORD n, LPCTSTR lpszSection)
{
	if (lpszSection != NULL)
		m_strSection = lpszSection;
	TCHAR szBuffer[MAX_PATH];
...
{
	if (bGet)
		n = GetDWORD(lpszEntry, nDefault, lpszSection);
	else
		WriteDWORD(lpszEntry, n, lpszSection);
}
...
bool CIni::GetBinary(LPCTSTR lpszEntry, BYTE **ppData, UINT *pBytes, LPCTSTR pszSection)
{
	...
	*pBytes = 0;
	...
	for (int i = 0; i < nLen; ++i) {
		if (str[i] < _T('A') || str[i] > _T('P'))
			return false;
	}

	const UINT uBytes = static_cast<UINT>(nLen / 2);
	BYTE *pb = new BYTE[uBytes];
	for (UINT i = 0; i < uBytes; ++i)
		pb[i] = static_cast<BYTE>(((str[i * 2 + 1] - _T('A')) << 4) + (str[i * 2] - _T('A')));
	*ppData = pb;
	*pBytes = uBytes;
	return true;
}
...
bool CIni::WriteBinary(LPCTSTR lpszEntry, LPBYTE pData, size_t nBytes, LPCTSTR lpszSection)
{
	if (pData == NULL && nBytes != 0)
		return false;
	if (nBytes > (INT_MAX - 1) / 2)
		return false;

	CString strValue;
	LPTSTR p = strValue.GetBuffer(static_cast<int>(nBytes * 2));
	for (; nBytes; --nBytes) {
		*p++ = static_cast<TCHAR>((*pData & 0x0F) + 'A'); //low nibble
		*p++ = static_cast<TCHAR>(((*pData++ >> 4) & 0x0F) + 'A'); //high nibble
	}
	strValue.ReleaseBuffer();

	WriteString(lpszEntry, strValue, lpszSection);
	return true;
}
