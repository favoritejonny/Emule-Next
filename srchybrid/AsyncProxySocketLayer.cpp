...
namespace
{
	uint16 ReadUInt16Unaligned(const void *source)
	{
		uint16 value;
	...
	uint32 ReadUInt32Unaligned(const void *source)
	{
		uint32 value;
		memcpy(&value, source, sizeof value);
		return value;
	...
	if (m_ProxyData.nProxyType == PROXYTYPE_SOCKS4 || m_ProxyData.nProxyType == PROXYTYPE_SOCKS4A) {
		if (m_nProxyOpState == 1) { //Both for PROXYOP_CONNECT and PROXYOP_BIND
			...
			if (m_nRecvBufferPos == 8) {
				TRACE(_T("SOCKS4 response: VN=%u  CD=%u  DSTPORT=%u  DSTIP=%s\n"), (BYTE)m_pRecvBuffer[0], (BYTE)m_pRecvBuffer[1], ntohs(ReadUInt16Unaligned(&m_pRecvBuffer[2])), (LPCTSTR)ipstr(ReadUInt32Unaligned(&m_pRecvBuffer[4])));
				...
				++m_nProxyOpState;
				uint32 ip = ReadUInt32Unaligned(&m_pRecvBuffer[4]);
				...
				data.ip = ip;
				data.nPort = static_cast<UINT>(ReadUInt16Unaligned(&m_pRecvBuffer[2]));
		...
		if (m_nProxyOpState == 2) { //Response to the auth request
			...
			if (m_nRecvBufferPos == m_nRecvBufferLen) {
				...
				if (m_nRecvBufferLen == 5) { //Check which kind of address the response contains
					...
					case 3: //FQDN
						{
							// The SOCKS5 domain length is an unsigned byte. Treating this
							// char as signed could under-allocate the receive buffer for
							// valid host names longer than 127 bytes.
							m_nRecvBufferLen += static_cast<uint8>(m_pRecvBuffer[4]) + 2;
				...
				if (m_nProxyOpID == PROXYOP_CONNECT) {
					...
					t_ListenSocketCreatedStruct data;
					data.ip = ReadUInt32Unaligned(&m_pRecvBuffer[4]);
					data.nPort = static_cast<UINT>(ReadUInt16Unaligned(&m_pRecvBuffer[8]));
					DoLayerCallback(LAYERCALLBACK_LAYERSPECIFIC, PROXYSTATUS_LISTENSOCKETCREATED, (LPARAM)&data);
				}
				ClearBuffer();
			}
			...
			m_nRecvBufferPos += numread;
			TRACE(_T("SOCKS5 response: VER=%u  REP=%u  RSV=%u  ATYP=%u  BND.ADDR=%s  BND.PORT=%u\n"), (BYTE)m_pRecvBuffer[0], (BYTE)m_pRecvBuffer[1], (BYTE)m_pRecvBuffer[2], (BYTE)m_pRecvBuffer[3], (LPCTSTR)ipstr(ReadUInt32Unaligned(&m_pRecvBuffer[4])), ntohs(ReadUInt16Unaligned(&m_pRecvBuffer[8])));
