...
public:
	...
	{
		if (!m_pfnMediaInfo_New || !m_pfnMediaInfo_Open || File == NULL)
			return NULL;
	...
	{
		if (!m_pfnMediaInfo_Get || Handle == NULL || Parameter == NULL)
			return CString();
		const wchar_t *pszValue = (*m_pfnMediaInfo_Get)(Handle, StreamKind, StreamNumber, Parameter, KindOfInfo, KindOfSearch);
		return pszValue ? CString(pszValue) : CString();
	}

	CString GetI(void *Handle, MediaInfo_stream_C StreamKind, size_t StreamNumber, size_t iParameter, MediaInfo_info_C KindOfInfo)
	{
		if (!m_pfnMediaInfo_GetI || Handle == NULL)
			return CString();
...
static size_t ID3_GetFieldValue(const ID3_Frame *frame, ID3_FieldID fldName)
{
	if (frame) {
...
static size_t ID3_GetFieldSize(const ID3_Frame *frame, ID3_FieldID fldName)
{
	if (frame) {
		ID3_Field *fld = frame->GetField(fldName);
		if (fld)
...
static size_t ID3_GetTextItemCount(const ID3_Frame *frame, ID3_FieldID fldName)
{
	if (frame) {
		ID3_Field *fld = frame->GetField(fldName);
		if (fld)
			return fld->GetNumTextItems();
	}
	return 0;
}
		...
		try {
			...
			for (const ID3_Frame *frame; (frame = iter->GetNext()) != NULL;) {
				...
				case ID3FID_INVOLVEDPEOPLE:
					{
						size_t nItems = ID3_GetTextItemCount(frame, ID3FN_TEXT);
				...
				case ID3FID_PICTURE:
					{
						...
						size_t nPicType = ID3_GetFieldValue(frame, ID3FN_PICTURETYPE);
						size_t nDataSize = ID3_GetFieldSize(frame, ID3FN_DATA);
				...
				case ID3FID_GENERALOBJECT:
					{
						...
						size_t
							nDataSize = ID3_GetFieldSize(frame, ID3FN_DATA);
				...
				case ID3FID_PLAYCOUNTER:
					{
						size_t nCounter = ID3_GetFieldValue(frame, ID3FN_COUNTER);
				...
				case ID3FID_POPULARIMETER:
					{
						...
						size_t nCounter = ID3_GetFieldValue(frame, ID3FN_COUNTER);
						size_t nRating = ID3_GetFieldValue(frame, ID3FN_RATING);
				...
				case ID3FID_GROUPINGREG:
					{
						wchar_t *sOwner = ID3_GetStringW(frame, ID3FN_OWNER);
						size_t nSymbol = ID3_GetFieldValue(frame, ID3FN_ID);
						size_t nDataSize = ID3_GetFieldSize(frame, ID3FN_DATA);
				...
				case ID3FID_SYNCEDLYRICS:
					{
						...
						size_t nRating = ID3_GetFieldValue(frame, ID3FN_CONTENTTYPE);
			...
			// id3lib 3.8.3 exposes an iterator without a virtual destructor, but its documented API requires this delete.
				...
				if (theMediaInfoDLL.Initialize()) {
					...
					if (m_handle) {
						...
						*/
						...
						double fFileLengthSec = static_cast<double>(_tstoi(str)) / SEC2MS(1.0);
						UINT uAllBitrates = 0;
						...
						if (iVideoStreams > 0) {
							...
							if (!str.IsEmpty()) {
								...
								if (strCodecA.GetLength() >= static_cast<int>(sizeof mi->video.bmiHeader.biCompression))
									memcpy(&mi->video.bmiHeader.biCompression, (LPCSTR)strCodecA, sizeof mi->video.bmiHeader.biCompression);
							}
						...
						if (mi->strFileFormat.Find(_T("MPEG")) == 0) {	/* MPEG container? */
								...
								&& uAllBitrates != _UI32_MAX)	/* do we have CBR only? */
							{
								...
								fFileLengthSec = static_cast<double>(pFile->GetFileSize()) * 8.0 / static_cast<double>(uAllBitrates);
								...
								if (mi->iVideoStreams > 0) {
									...
									if (mi->fVideoFrameRate > 0) {
										...
										const uint64 uFrames = static_cast<uint64>(fFileLengthSec * mi->fVideoFrameRate);
										if (uFrames <= uFileSize / 24)
											fFileLengthSec = static_cast<double>(uFileSize - uFrames * 24) * 8.0 / static_cast<double>(uAllBitrates);
									}
									mi->fVideoLengthSec = fFileLengthSec;
								}
									...
									if (SUCCEEDED(pMediaDet->get_StreamType(&major_type))) {
										if (major_type == MEDIATYPE_Video) {
											...
											if (SUCCEEDED(pMediaDet->get_StreamMediaType(&mt))) {
												if (mt.formattype == FORMAT_VideoInfo && mt.pbFormat != NULL && mt.cbFormat >= sizeof(VIDEOINFOHEADER)) {
													VIDEOINFOHEADER *pVIH = (VIDEOINFOHEADER*)mt.pbFormat;

													if (mi->iVideoStreams == 1) {
														memcpy(&mi->video, pVIH, sizeof mi->video);
											...
											AM_MEDIA_TYPE mt = {};
											if (SUCCEEDED(pMediaDet->get_StreamMediaType(&mt))) {
												if (mt.formattype == FORMAT_WaveFormatEx && mt.pbFormat != NULL && mt.cbFormat >= sizeof(WAVEFORMATEX)) {
