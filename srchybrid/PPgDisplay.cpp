...
BEGIN_MESSAGE_MAP(CPPgDisplay, CPropertyPage)
	...
	ON_CBN_SELCHANGE(IDC_NEXT_THEME, OnCbnSelchangeNextTheme)
	ON_WM_HELPINFO()
END_MESSAGE_MAP()
...
void CPPgDisplay::LoadSettings()
{
	...
	if (m_cbNextTheme.m_hWnd)
		m_cbNextTheme.SetCurSel((int)CNextTheme::GetTheme());
}
...
BOOL CPPgDisplay::OnInitDialog()
{
	...
	// The old 3D-depth slider only changed legacy bevels. Reuse its compact
	// location for the built-in themes while retaining the rest of this page.
	CRect rcTheme;
	GetDlgItem(IDC_3DDEPTH)->GetWindowRect(&rcTheme);
	ScreenToClient(&rcTheme);
	rcTheme.bottom = rcTheme.top + 180;
	GetDlgItem(IDC_3DDEPTH)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_FLAT)->ShowWindow(SW_HIDE);
	GetDlgItem(IDC_ROUND)->ShowWindow(SW_HIDE);
	m_3DPreview.ShowWindow(SW_HIDE);
	m_cbNextTheme.Create(WS_CHILD | WS_VISIBLE | WS_TABSTOP | CBS_DROPDOWNLIST | WS_VSCROLL,
		rcTheme, this, IDC_NEXT_THEME);
	m_cbNextTheme.SetFont(GetFont());
	m_cbNextTheme.AddString(_T("eMule Next - Modern light"));
	m_cbNextTheme.AddString(_T("eMule Next - Aurora light"));
	m_cbNextTheme.AddString(_T("Classic Windows"));
...
BOOL CPPgDisplay::OnApply()
{
	...
	const EEmuleNextTheme ePreviousTheme = CNextTheme::GetTheme();
	int iTheme = m_cbNextTheme.GetCurSel();
	if (iTheme == CB_ERR)
		iTheme = entModernLight;
	CNextTheme::SetTheme((EEmuleNextTheme)iTheme);
	...
	if (!thePrefs.ShowRatesOnTitle())
		theApp.emuledlg->SetWindowText(theApp.GetProductDisplayName());
	if (ePreviousTheme != CNextTheme::GetTheme())
		theApp.emuledlg->RedrawWindow(NULL, NULL, RDW_ERASE | RDW_FRAME | RDW_INVALIDATE | RDW_ALLCHILDREN | RDW_UPDATENOW);
...
void CPPgDisplay::OnCbnSelchangeNextTheme()
{
	SetModified();
}
