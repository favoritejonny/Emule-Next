...
CString GetVideoFormatName(DWORD biCompression)
{
	...
	if (strFormat.IsEmpty()) {
		char szFourcc[5];
		memcpy(szFourcc, &biCompression, sizeof biCompression);
		szFourcc[4] = '\0';
		strFormat = szFourcc;
		strFormat.MakeUpper();
	}
...
template <typename T>
static T ReadUnalignedValue(const void *pValue) noexcept
{
	T value;
...
static bool SkipFileBytes(int fd, DWORD dwBytes)
{
	return _lseeki64(fd, static_cast<__int64>(dwBytes), SEEK_CUR) >= 0;
}

static bool SkipRiffChunk(int fd, DWORD dwLength)
{
	return _lseeki64(fd, static_cast<__int64>(dwLength) + (dwLength & 1), SEEK_CUR) >= 0;
}
...
static bool ParseStreamHeader(int hAviFile, DWORD dwLengthLeft, STREAMHEADER *pStrmHdr)
{
	...
	while (dwLengthLeft >= sizeof(DWORD) * 2) {
		...
		const DWORD dwPaddedLength = dwLength + (dwLength & 1);
		if (dwPaddedLength < dwLength || dwPaddedLength > dwLengthLeft) {
			errno = 0;
			return false;
		}
		dwLengthLeft -= dwPaddedLength;
		...
		case ckidSTREAMHEADER:
			if (dwLength < sizeof pStrmHdr->hdr) {
				...
				if (_read(hAviFile, &pStrmHdr->hdr, dwLength) != (int)dwLength)
					return false;
				if (dwLength & 1 && !SkipFileBytes(hAviFile, 1))
					return false;
			} else {
				if (_read(hAviFile, &pStrmHdr->hdr, sizeof pStrmHdr->hdr) != sizeof pStrmHdr->hdr)
					return false;
				if (!SkipFileBytes(hAviFile, dwLength + (dwLength & 1) - sizeof pStrmHdr->hdr))
					return false;
			}
			dwLength = 0;
			break;
		...
		if (dwLength && !SkipRiffChunk(hAviFile, dwLength))
			return false;
	}

	return !(dwLengthLeft && !SkipFileBytes(hAviFile, dwLengthLeft));
}
...
bool GetRIFFHeaders(LPCTSTR pszFileName, SMediaInfo *mi, bool &rbIsAVI, bool bFullInfo)
{
	...
	while (!bHaveReadAllStreams && dwLengthLeft >= sizeof(DWORD) * 2) {
						...
						if (strmhdr.hdr.fccType == streamtypeAUDIO) {
							...
							if (mi->iAudioStreams == 1) {
								...
								if (strmhdr.fmt.wav && strmhdr.dwFormatLen >= sizeof *strmhdr.fmt.wav) {
									memcpy(&mi->audio, &strmhdr.fmt.wav->wf, sizeof mi->audio);
		...
		if (bHaveReadAllStreams)
			break;
		if (dwLength) {
			if (!SkipRiffChunk(hAviFile, dwLength))
				goto inv_format_errno;
		}
	}
	...
	if (fccMain == formtypeAVI) {
		...
		if (mi->iVideoStreams == 1 /*&& mi->iAudioStreams <= 1*/ && iNonAVStreams == 0 && mi->fVideoLengthSec) {
			const uint64 uVideoFramesOverhead = static_cast<uint64>(uVideoFrames) * (sizeof(WORD) + sizeof(WORD) + sizeof(DWORD));
			if (uVideoFramesOverhead < dwMovieChunkSize) {
				const double fPayloadBytesPerSec = (static_cast<double>(dwMovieChunkSize) - static_cast<double>(uVideoFramesOverhead)) / mi->fVideoLengthSec;
				if (fPayloadBytesPerSec > dwAllNonVideoAvgBytesPerSec) {
					const double fVideoBitRate = (fPayloadBytesPerSec - dwAllNonVideoAvgBytesPerSec) * 8.0;
					mi->video.dwBitRate = fVideoBitRate >= _UI32_MAX ? _UI32_MAX : static_cast<DWORD>(fVideoBitRate);
				}
			}
		}
...
{
	CString strInfo(GetFOURCCString(ReadUnalignedValue<DWORD>(pszCodecID)));
					...
					if (strMimeType == "video/x-pn-realvideo") {
						...
						if (mi->iVideoStreams == 1) {
							...
							if (dwTypeDataLen >= 22 + 2 && dwTypeDataLen < 8192) {
								...
								try {
									...
									if (_byteswap_ulong(ReadUnalignedValue<DWORD>(pucTypeData + 4)) == 'VIDO') {
										mi->strVideoFormat = GetRealMediaCodecInfo((LPCSTR)(pucTypeData + 8));
										mi->video.bmiHeader.biCompression = ReadUnalignedValue<DWORD>(pucTypeData + 8);
										mi->video.bmiHeader.biWidth = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 12));
										mi->video.bmiHeader.biHeight = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 14));
										if (mi->video.bmiHeader.biHeight != 0)
											mi->fVideoAspectRatio = fabs(mi->video.bmiHeader.biWidth / static_cast<double>(mi->video.bmiHeader.biHeight));
										mi->fVideoFrameRate = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 22));
						...
						if (mi->iAudioStreams == 1) {
							...
							if (dwTypeDataLen >= 4 + 2 && dwTypeDataLen < 8192) {
								...
								try {
									file.Read(pucTypeData, dwTypeDataLen);
									DWORD dwFourCC = ReadUnalignedValue<DWORD>(pucTypeData);
									WORD wVer = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 4));
									if (dwFourCC == MAKEFOURCC('.', 'r', 'a', '\xFD')) {
										if (wVer == 3) {
											if (dwTypeDataLen >= 8 + 2) {
												mi->audio.nSamplesPerSec = 8000;
												mi->audio.nChannels = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 8));
												mi->strAudioFormat = _T(".ra3");
												bReadMDPR_Audio = true;
											}
										} else if (wVer == 4) { // RealAudio G2, RealAudio 8
											if (dwTypeDataLen >= 62 + 4) {
												mi->audio.nSamplesPerSec = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 48));
												mi->audio.nChannels = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 54));
											...
											if (dwTypeDataLen >= 66 + 4) {
												mi->audio.nSamplesPerSec = _byteswap_ulong(ReadUnalignedValue<DWORD>(pucTypeData + 48));
												mi->audio.nChannels = _byteswap_ushort(ReadUnalignedValue<WORD>(pucTypeData + 60));
												if (strncmp((LPCSTR)(pucTypeData + 62), "sipr", 4) == 0)
													mi->audio.wFormatTag = WAVE_FORMAT_SIPROLAB_ACEPLNET;
												else if (strncmp((LPCSTR)(pucTypeData + 62), "cook", 4) == 0)
													mi->audio.wFormatTag = 0x2004;
						...
						if (wVersion == 0) {
							...
							while (wProperties) {
								...
								if (wPropVersion == 0) {
									...
									if (dwPropSize < sizeof(DWORD) + sizeof(WORD)) {
										bBrokenFile = true;
										break;
									}
							...
							if (pStreamTypeInfo->guidMajorType == WMMEDIATYPE_Video) {
									...
									&& pStreamTypeInfo->cbFormat >= sizeof(WMVIDEOINFOHEADER))
								{
									...
									if (pVideoInfo->bmiHeader.biSize >= sizeof pVideoInfo->bmiHeader) {
										if (mi->iVideoStreams == 1) {
											memcpy(&mi->video, pVideoInfo, sizeof mi->video);
											...
											if (hr == S_OK) {
												...
												if (codecType == WMT_CODECINFO_AUDIO) {
													if (wCodecInfoSize == sizeof(WORD))
														dwCodecId = ReadUnalignedValue<WORD>((BYTE*)aCodecInfo);
													else
														ASSERT(0);
												} else if (codecType == WMT_CODECINFO_VIDEO) {
													if (wCodecInfoSize == sizeof(DWORD))
														dwCodecId = ReadUnalignedValue<DWORD>((BYTE*)aCodecInfo);
													else
														ASSERT(0);
												} else
													ASSERT(0);
											...
											if (bHasAudio && codecType == WMT_CODECINFO_AUDIO) {
												if (wCodecInfoSize == sizeof(WORD)) {
													if (!bAddedBakedAudioStream) {
														...
														++mi->iAudioStreams;
														mi->audio.wFormatTag = ReadUnalignedValue<WORD>((BYTE*)aCodecInfo);
