...
bool CKnownFile::CreateFromFile(LPCTSTR in_directory, LPCTSTR in_filename, LPVOID pvProgressParam)
{
	...
	// open file
	const CString strFilePath = MakePath(NULL, in_directory, in_filename, NULL);
	if (strFilePath.IsEmpty()) {
		LogError(GetResString(IDS_ERR_FILEOPEN), in_filename, _T(""));
		return false;
	}
	SetFilePath(strFilePath);
	const CString strFileSystemPath = MakeLongPathCompatible(strFilePath);
	FILE *file = _tfsopen(strFileSystemPath, _T("rbS"), _SH_DENYNO); // can not use _SH_DENYWR because we may access a completing part file
	if (!file) {
...
bool CKnownFile::CreateAICHHashSetOnly()
{
	...
	const CString strFileSystemPath = MakeLongPathCompatible(GetFilePath());
	FILE *file = _tfsopen(strFileSystemPath, _T("rbS"), _SH_DENYNO); // can not use _SH_DENYWR because we may access a completing part file
	if (!file) {
