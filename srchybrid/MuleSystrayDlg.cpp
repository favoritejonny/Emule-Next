...
BOOL CMuleSystrayDlg::OnInitDialog()
{
	...
	const COLORREF crBackground = eTheme == entModernAurora ? RGB(244, 249, 250)
		: (eTheme == entClassic ? ::GetSysColor(COLOR_BTNFACE) : RGB(248, 250, 252));
	m_brBackground.DeleteObject();
	m_brBackground.CreateSolidBrush(crBackground);
	...
	if (p) {
		...
		m_ctrlSpeed.m_sIcon.cx = 18;
		m_ctrlSpeed.m_sIcon.cy = 18;
	...
	if (p) {
		...
		m_ctrlAllToMax.m_sIcon.cx = 18;
		m_ctrlAllToMax.m_sIcon.cy = 18;
	...
	if (p) {
		...
		m_ctrlAllToMin.m_sIcon.cx = 18;
		m_ctrlAllToMin.m_sIcon.cy = 18;
	...
	if (p) {
		...
		m_ctrlRestore.m_sIcon.cx = 18;
		m_ctrlRestore.m_sIcon.cy = 18;
	...
	if (p) {
		...
		m_ctrlConnect.m_sIcon.cx = 18;
		m_ctrlConnect.m_sIcon.cy = 18;
	...
	if (p) {
		...
		m_ctrlDisconnect.m_sIcon.cx = 18;
		m_ctrlDisconnect.m_sIcon.cy = 18;
	...
	if (p) {
		...
		m_ctrlPreferences.m_sIcon.cx = 18;
		m_ctrlPreferences.m_sIcon.cy = 18;
	...
	if (p) {
		...
		m_ctrlExit.m_sIcon.cx = 18;
		m_ctrlExit.m_sIcon.cy = 18;
	...
	CFont Font;
	Font.CreateFont(-16, 0, 900, 0, FW_SEMIBOLD, 0, 0, 0, DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY,
		DEFAULT_PITCH | FF_DONTCARE, _T("Segoe UI"));

	if (eTheme == entModernLight)
		m_ctrlSidebar.SetColors(RGB(255, 255, 255), RGB(48, 139, 204), RGB(96, 187, 219));
	else if (eTheme == entModernAurora)
		m_ctrlSidebar.SetColors(RGB(255, 255, 255), RGB(43, 151, 160), RGB(111, 198, 186));
	else {
		const int iClr = g_bLowColorDesktop ? COLOR_ACTIVECAPTION : COLOR_GRADIENTACTIVECAPTION;
	...
	// Show the public eMule Next identity here. m_strCurVersionLong is the
	// legacy protocol/base version and must not appear as the product name.
	m_ctrlSidebar.SetWindowText(CemuleApp::GetProductDisplayName());

	// The old tray panel used etched separators from the Windows 9x era. The
	// modern buttons provide their own spacing and hover outline, so those
	// decorative controls are intentionally hidden without changing commands.
	for (CWnd *pChild = GetWindow(GW_CHILD); pChild != NULL; pChild = pChild->GetNextWindow()) {
		TCHAR szClass[16] = {};
		::GetClassName(pChild->m_hWnd, szClass, _countof(szClass));
		if (_tcsicmp(szClass, _T("Static")) == 0 && (pChild->GetStyle() & SS_TYPEMASK) == SS_ETCHEDHORZ)
...
BOOL CMuleSystrayDlg::OnEraseBkgnd(CDC *pDC)
{
	CRect rc;
	GetClientRect(&rc);
	pDC->FillRect(&rc, m_brBackground.GetSafeHandle() ? &m_brBackground : CBrush::FromHandle(::GetSysColorBrush(COLOR_BTNFACE)));
	return TRUE;
}

HBRUSH CMuleSystrayDlg::OnCtlColor(CDC *pDC, CWnd *pWnd, UINT nCtlColor)
{
	HBRUSH hBrush = CDialog::OnCtlColor(pDC, pWnd, nCtlColor);
	if (CNextTheme::GetTheme() == entClassic || !m_brBackground.GetSafeHandle())
		return hBrush;

	if (nCtlColor == CTLCOLOR_STATIC || nCtlColor == CTLCOLOR_DLG) {
		pDC->SetBkMode(TRANSPARENT);
		pDC->SetTextColor(CNextTheme::GetTheme() == entModernAurora ? RGB(31, 58, 67) : RGB(35, 49, 64));
