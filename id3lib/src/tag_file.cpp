...
size_t RenderV2ToFile(const ID3_TagImpl& tag, fstream& file)
{
  ...
  else
  {
    ...
    if (filename.size() > ID3_PATH_LENGTH - sTmpSuffix.size() - 1)
    {
      // log this
      return 0;
    ...
    const size_t tempFileLength = filename.size() + sTmpSuffix.size();
    memcpy(sTempFile, filename.data(), filename.size());
    memcpy(sTempFile + filename.size(), sTmpSuffix.data(), sTmpSuffix.size());
    sTempFile[tempFileLength] = '\0';
