...
void ID3_FrameHeader::SetUnknownFrame(const char* id)
{
  ...
  _dyn_frame_def = true;
  memset(_frame_def->sShortTextID, 0, sizeof(_frame_def->sShortTextID));
  memset(_frame_def->sLongTextID, 0, sizeof(_frame_def->sLongTextID));
  if (id == NULL)
  {
    return;
  }
  ...
  if (idLength <= 3)
  {
    memcpy(_frame_def->sShortTextID, id, idLength);
  }
  else if (idLength <= 4)
  {
    memcpy(_frame_def->sLongTextID, id, idLength);
  }
}
...
ID3_FrameHeader& ID3_FrameHeader::operator=(const ID3_FrameHeader& hdr)
{
  if (this != &hdr)
  {
    ...
    else
    {
      ...
      memcpy(_frame_def->sShortTextID, hdr._frame_def->sShortTextID, sizeof(_frame_def->sShortTextID));
      memcpy(_frame_def->sLongTextID, hdr._frame_def->sLongTextID, sizeof(_frame_def->sLongTextID));
      _dyn_frame_def = true;
    }
  }
  return *this;
