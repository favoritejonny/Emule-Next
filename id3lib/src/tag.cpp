...
{
  if (buffer == NULL || bytes > (std::numeric_limits<ID3_Reader::size_type>::max)())
  {
    return 0;
  }

  ID3_MemoryReader mr(buffer, static_cast<ID3_Reader::size_type>(bytes));
...
const char* ID3_Tag::GetFileName() const
{
  ...
  if (fn.size() && fn.size() < ID3_PATH_LENGTH)
  {
    memset((char *)_tmp_filename, 0, ID3_PATH_LENGTH);
    memcpy((char *)_tmp_filename, fn.data(), fn.size());
...
void ID3_Tag::AddFrames(const ID3_Frame *frames, size_t numFrames)
{
  ...
  while (numFrames > 0)
  {
    this->AddFrame(frames[--numFrames]);
  }
}
...
int32 ID3_IsTagHeader(const uchar data[ID3_TAGHEADERSIZE])
{
  ...
  if (size < ID3_TagHeader::SIZE)
  {
    return -1;
  }

  const size_t payloadSize = size - ID3_TagHeader::SIZE;
  if (payloadSize > static_cast<size_t>((std::numeric_limits<int32>::max)()))
  {
    return -1;
  }
  return static_cast<int32>(payloadSize);
}
