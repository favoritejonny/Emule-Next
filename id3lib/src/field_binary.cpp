                             ...
                             )
{
  ...
  FILE* temp_file = OpenBinaryFile(info, "rb");
  if (temp_file != NULL)
  {
    if (::fseek(temp_file, 0, SEEK_END) == 0)
    {
      const long fileLength = ::ftell(temp_file);
      if (fileLength > 0 && ::fseek(temp_file, 0, SEEK_SET) == 0)
      {
        const size_t fileSize = static_cast<size_t>(fileLength);
        std::unique_ptr<uchar[]> buffer(new (std::nothrow) uchar[fileSize]);
        if (buffer != NULL && ::fread(buffer.get(), 1, fileSize, temp_file) == fileSize)
        {
          this->Set(buffer.get(), fileSize);
        }
      }
    }
                           ...
                           ) const
{
  ...
  if (size > 0)
  {
    FILE* temp_file = OpenBinaryFile(info, "wb");
    if (temp_file != NULL)
    {
